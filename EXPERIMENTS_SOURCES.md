...
| fig:app-c16 | `figA2_worst_head_budgets_{adult,crime,acs}_c16.pdf` | `results_summary/appendix_paper/` (the c=4 figures were dropped 2026-09-13; three-panel appendix figures are rendered at 7.4×2.15 in so two fit per page) |
...
| fig:app-gdp / fig:app-hgr / fig:app-supipm / fig:app-gdp-binned / fig:app-mi-y | `figA6_other_measures_{adult,crime,acs}_{gdp_w_kernel,hgr,sup_ipm,gdp_wo_kernel,mi_y_s}.pdf` | `results_summary/appendix_paper/` (MI via the k-NN estimator ported from FREM, `src/evaluate.py:compute_MI`; binned GDP = FREM's `gdp_wo_kernel` over the observed values of S; the $I(\mathbf{Z};S)$ figure was removed 2026-09-13 at the author's request) |
