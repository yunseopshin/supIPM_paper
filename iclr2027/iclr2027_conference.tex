...
Moreover, a mini-batch learning is not possible for supIPM with MMD \citep{lee2026fair}, and thus  
$O(n^2)$ memory is required. In contrast, we show in Appendix~\ref{app:mini-batch} that supIPM with the ReLU-IPM admits a mini-batch learning.

Theoretically, the class of discriminators for ReLU-IPM is much larger that that of MMD, Thus, controlling supIPM with $\operatorname{IPM}_{\mathcal{V}_{\mathrm{R}}}$
can guarantee the level of fairness violation for large class of prediction heads. Empirical studies in Section \ref{sec:experiments} confirm this advantage.
...
\paragraph{Baselines.}
We compare supIPM-FRL with seven baselines for fair representation learning (FRL) algoritms or for learning fair prediction models.
...
This appendix examines three choices in the supIPM-FRL on the tabular datasets: the kernel in estimator of distribution, the class of the discriminators and the number $T$ of candidate sensitive values. 
