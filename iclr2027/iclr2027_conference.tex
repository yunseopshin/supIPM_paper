...
This section evaluates supIPM-FRL on simulated data, in which the sensitive value $s_0$ at which the dependence on $S$ is concentrated is known, and on three tabular datasets, where we compare it with existing baselines. 
...
\paragraph{Measures.}  Prediction performance is measured by the test accuracy for classification and by the test $1-\mathrm{MAE}$ for regression, as in \citet{kong2025fair}.  Fairness is measured by the prediction-level disparity $\operatorname{GDP}_{\infty}(f_{\psi}\circ h_{\phi})$ in Equation~(\ref{eq:linfty-gdp}), the disparity of the head trained jointly with the encoder.  Since a representation may later serve other prediction heads, we also measure the largest disparity that a downstream head can attain on it, following \citet{coston2021characterizing,jovanovic2023fare,langlade2025fairness,luo2026fair}.  For $c>0$, let $\mathcal{F}_c:=\{\sigma(\mathbf{w}^\top\mathbf{z}+b):\lVert\mathbf{w}\rVert_2\le c,\ b\in\mathbb{R}\}$, where $\sigma$ is the logistic function, and define the worst-case downstream disparity
\begin{equation}
  \operatorname{GDP}_{\infty}(h;\mathcal{F}_c):=\sup_{f\in\mathcal{F}_c}\operatorname{GDP}_{\infty}(f\circ h).
  \label{eq:worst-case-gdp}
\end{equation}
Every head in $\mathcal{F}_c$ has a bounded H\"older norm, so Corollary~\ref{cor:certificate} bounds Equation~(\ref{eq:worst-case-gdp}) restricted to the interior region $\mathcal{S}_n$ in terms of the empirical supIPM.  For a head $f$ we write $\Delta(s):=|\mathbb{E}[f(\mathbf{Z})\mid S=s]-\mathbb{E}[f(\mathbf{Z})]|$, so that $\operatorname{GDP}_{\infty}(f\circ h)=\sup_{s}\Delta(s)$.  Every disparity reported below is a kernel estimate over $\mathcal{S}_n$, written with a hat as $\widehat{\Delta}(s)$ and $\widehat{\operatorname{GDP}}_{\infty}$ and defined in Appendix~\ref{app:worst-case}.

\paragraph{Computation of the worst-case disparity.}  The supremum in Equation~(\ref{eq:worst-case-gdp}) is approximated by a head of $\mathcal{F}_c$ found with the encoder held fixed.  With $\widehat{\operatorname{GDP}}_{\infty}$ and the empirical supervised risk $\widehat{\mathcal{R}}$ computed on the training data, for $\lambda_h>0$ we find
\begin{equation}
  \hat{f}_c:=\argmin_{f\in\mathcal{F}_c}\Bigl\{\widehat{\mathcal{R}}(f\circ h)-\lambda_h\,\widehat{\operatorname{GDP}}_{\infty}(f\circ h)\Bigr\}
  \label{eq:worst-case-estimate}
\end{equation}
by projected gradient descent over $(\mathbf{w},b)$, and estimate Equation~(\ref{eq:worst-case-gdp}) by the disparity of this head on the test data, $\widehat{\operatorname{GDP}}_{\infty}(h;\mathcal{F}_c):=\widehat{\operatorname{GDP}}_{\infty}(\hat{f}_c\circ h)$.  With $\lambda_h=10^{3}$ the disparity dominates the objective, so that $\hat{f}_c$ approximates a head attaining the supremum in Equation~(\ref{eq:worst-case-gdp}); the optimization is described in Appendix~\ref{app:worst-case}.
...
We generate $S$ uniformly on an interval; a single input variable depends on $S$ through a term concentrated near a value $s_0$, and the outcome $Y$ does not depend on $S$, so this dependence can be removed without loss of prediction performance; the full setup is given in Appendix~\ref{app:synth-experiments}.
We compare supIPM-FRL with FREM over a grid of penalty parameters, with the worst-case downstream disparity over $\mathcal{F}_1$ as the fairness measure; each curve joins the configurations in increasing order of the penalty parameter.

\begin{figure}[!h]
  \centering
  \includegraphics[width=0.7\textwidth]{main/fig1_simulation.pdf}
  \caption{Left: prediction performance against $\widehat{\operatorname{GDP}}_{\infty}(h_{\phi};\mathcal{F}_1)$; the upper left is favorable.  Right: $\widehat{\Delta}(s)$ of the heads $\hat{f}_1$ at the two circled points.}
...
At the circled points, the configurations with the smallest disparity that retain the prediction performance of the model trained without a penalty, FREM still exhibits a peak in $\widehat{\Delta}(s)$ at $s_0$, whereas the $\widehat{\Delta}(s)$ of supIPM-FRL at $s_0$ is markedly smaller and not distinguishable from its values elsewhere. 
...
\paragraph{Baselines.}  Four baselines use the continuous $S$ directly: FREM \citep{kong2025fair} penalizes the representation by the EIPM of Section~\ref{sec:frem}, Reg-GDP \citep{jiang2022generalized} and Reg-HGR \citep{mary2019fairness} penalize the prediction by an estimate of the GDP of Equation~(\ref{eq:gdp}) and of the HGR maximal correlation, respectively, and ADV \citep{zhang2018mitigating,kong2025fair} trains an adversary to predict $S$ from the representation.  Three baselines are designed for a binary sensitive attribute and align the representations of two groups: LAFTR \citep{madras2018learning}, MMD \citep{deka2023mmd} and sIPM-LFR \citep{kim2022learning}.  They are trained with $S$ discretized at its median and evaluated with the continuous $S$ as every other method.  All methods share the architecture of Appendix~\ref{app:real-settings} and are trained over a grid of penalty parameters with five random seeds.

\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.72\textwidth]{main/fig2m_own_head_adult.pdf}
  \caption{Adult: test accuracy against the prediction-level disparity $\widehat{\operatorname{GDP}}_{\infty}(f_{\psi}\circ h_{\phi})$.}
  \label{fig:adult}
  \vspace{-10pt}
\end{figure}

\paragraph{Results.}  Each point is the mean over the five seeds with bars of $\pm1$ standard deviation, each curve joins the configurations of a method with the smallest disparity at each level of prediction performance, and the upper left is favorable.  Figure~\ref{fig:adult} shows the prediction-level disparity on Adult, and Figure~\ref{fig:worst-case} the worst-case downstream disparity over $\mathcal{F}_1$ on the three datasets; Appendix~\ref{app:real-tabular} gives the prediction-level curves on Crime and ACSIncome, which show the same pattern, and the worst-case curves over $\mathcal{F}_4$ and $\mathcal{F}_8$.  At the prediction level, supIPM-FRL is close to FREM and Reg-GDP over the accuracies it attains, and the methods that discretize $S$ lie below or close to its curve.  Under the worst-case downstream disparity, supIPM-FRL attains a markedly smaller disparity than every other method at the same prediction performance on all three datasets, whether the method uses $S$ directly or discretizes it, except at the lowest prediction performance on Crime, where the methods that use $S$ directly coincide with it.

\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.92\textwidth]{main/fig3g_worst_head_all.pdf}
  \caption{Adult, Crime and ACSIncome: prediction performance against the worst-case downstream disparity $\widehat{\operatorname{GDP}}_{\infty}(h_{\phi};\mathcal{F}_1)$.}
  \label{fig:worst-case}
  \vspace{-10pt}
\end{figure}

The prediction-level disparity concerns the trained head only, whereas the worst-case disparity concerns every head in $\mathcal{F}_1$.  A penalty on the prediction, as in Reg-GDP and Reg-HGR, or on a single adversary, as in ADV and LAFTR, constrains only the dependence on $S$ that this head or adversary detects, and a downstream head can exploit the rest.  A penalty averaged over $S$, as in FREM, can leave dependence at sensitive values of small probability, and a penalty on the two groups of a discretized $S$, as in MMD and sIPM-LFR, can leave dependence on $S$ within each group.  supIPM-FRL instead constrains the discrepancy between $\mathbb{P}_{\mathbf{Z}\mid S=s}$ and $\mathbb{P}_{\mathbf{Z}}$ at every $s$ over all ReLU discriminators, so the fairness it attains extends to every head in $\mathcal{F}_1$, in accordance with Corollary~\ref{cor:certificate}.  The empirical supIPM on the test data, given in Appendix~\ref{app:real-measures}, is smallest for supIPM-FRL on Adult and Crime at a matched level of prediction performance, although FREM attains a smaller value on ACSIncome.
...
\paragraph{Prediction and fairness measures.}  All reported measures are computed on the test data, indexed here by $i=1,\ldots,n$; Equation~(\ref{eq:worst-case-estimate}) applies the same formulas to the training sample.  Prediction performance is the accuracy for classification and $1-\mathrm{MAE}$ for regression.  For both tasks the prediction $f_{\psi}(\mathbf{z})$ is the logistic function of the linear head of Appendix~\ref{app:real-settings}, so that $f_{\psi}\circ h_{\phi}$ and the heads of $\mathcal{F}_c$ take values in $(0,1)$.  Fairness is measured by the estimates $\widehat{\operatorname{GDP}}_{\infty}(f_{\psi}\circ h_{\phi})$ and $\widehat{\operatorname{GDP}}_{\infty}(h_{\phi};\mathcal{F}_c)$ of the prediction-level and the worst-case downstream disparity of Section~\ref{sec:experiments}, with $c=1$ except in Appendix~\ref{app:real-tabular}.  Both use the estimates
\begin{equation}
...
\end{equation}
where $\widehat{w}_{\tilde{\gamma}}(i;s)$ is the kernel weight of Equation~(\ref{eq:supipm-estimators}) with a Gaussian kernel of bandwidth $\tilde{\gamma}=(3n/4)^{-1/5}$ \citep{silverman2018density}, so that the first sum is the Nadaraya--Watson estimate of $\mathbb{E}[f(\mathbf{Z})\mid S=s]$, and $\mathcal{Q}$ is a grid of spacing $10^{-3}$ between the $5\%$ and the $95\%$ quantiles of $S$ on the training data, the empirical counterpart of the interior region $\mathcal{S}_n$ of Section~\ref{sec:theory}.

\paragraph{Worst-case downstream disparity.}  The head $\hat{f}_c$ of Equation~(\ref{eq:worst-case-estimate}) is computed with $\widehat{\operatorname{GDP}}_{\infty}$ of Equation~(\ref{eq:gdp-estimate}) and the empirical supervised risk $\widehat{\mathcal{R}}$ formed from the training sample, by projected gradient descent over $(\mathbf{w},b)$ from eight initial heads, with $\mathbf{w}$ projected onto the ball of radius $c$ after every step.  The reported value $\widehat{\operatorname{GDP}}_{\infty}(h;\mathcal{F}_c)=\widehat{\operatorname{GDP}}_{\infty}(\hat{f}_c\circ h)$ of Section~\ref{sec:experiments} is computed on the test sample.
...
Every curve in Section~\ref{sec:exp-real} and in this appendix retains, at each level of prediction performance, the smallest disparity over the evaluated penalty values and settings, and shows the mean over five seeds with $\pm1$ standard deviation.  Runs whose encoder degenerates to a constant predictor are excluded, and a configuration is retained when at least two seeds remain.
