...
\paragraph{Measures.}  Prediction performance is the test accuracy for classification and the test $1-\mathrm{MAE}$ for regression, as in \citet{kong2025fair}.  The prediction-level disparity is $\operatorname{GDP}_{\infty}(f_{\psi}\circ h_{\phi})$ of Equation~(\ref{eq:linfty-gdp}) for the head trained jointly with the encoder.  Since a representation may later serve other heads \citep{coston2021characterizing,jovanovic2023fare,langlade2025fairness,luo2026fair}, we also measure the largest disparity that a downstream head can attain.  For $c>0$, let $\mathcal{F}_c:=\{\sigma(\mathbf{w}^\top\mathbf{z}+b):\lVert\mathbf{w}\rVert_2\le c,\ b\in\mathbb{R}\}$ be the logistic heads with weight norm at most $c$, and define the worst-case downstream disparity
\begin{equation}
...
\paragraph{Estimating the worst-case disparity.}  With the encoder held fixed, and with $\widehat{\operatorname{GDP}}_{\infty}$ and the empirical supervised risk $\widehat{\mathcal{R}}$ computed on the training data, we find
\begin{equation}
...
\end{equation}
by projected gradient descent over $(\mathbf{w},b)$, with $\lambda_h=10^{3}$ so that the disparity term dominates and $\hat{f}_c$ approximates the head attaining the supremum in Equation~(\ref{eq:worst-case-gdp}).  The reported value is the disparity of this head on the test data, $\widehat{\operatorname{GDP}}_{\infty}(h;\mathcal{F}_c):=\widehat{\operatorname{GDP}}_{\infty}(\hat{f}_c\circ h)$; the optimization is described in Appendix~\ref{app:worst-case}.
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.8\textwidth]{main/fig2m_own_head_adult.pdf}
  \caption{Adult: test accuracy against the prediction-level disparity $\widehat{\operatorname{GDP}}_{\infty}(f_{\psi}\circ h_{\phi})$.}
  \label{fig:adult}
  \vspace{-12pt}
\end{figure}

\paragraph{Results.}  Each point is the mean over five seeds with bars of $\pm1$ standard deviation, each curve traces the configurations of one method with the smallest disparity at each level of prediction performance, and the upper left is favorable.  Figure~\ref{fig:adult} shows the prediction-level disparity on Adult, and Figure~\ref{fig:worst-case} the worst-case downstream disparity over $\mathcal{F}_1$ on the three datasets; the prediction-level curves on Crime and ACSIncome and the worst-case curves over $\mathcal{F}_4$ and $\mathcal{F}_8$ are given in Appendix~\ref{app:real-tabular}.  At the prediction level, supIPM-FRL is comparable to the other methods.  Under the worst-case downstream disparity, supIPM-FRL attains a markedly smaller disparity than every other method at the same prediction performance on all three datasets, except at the lowest prediction performance on Crime, where the methods that use $S$ directly coincide with it.
...
The two measures differ because the prediction-level disparity concerns the trained head only, whereas the worst-case disparity concerns every head in $\mathcal{F}_1$.  A penalty on the prediction (Reg-GDP, Reg-HGR) or on a single adversary (ADV, LAFTR) constrains only the dependence on $S$ that this head or adversary detects, a penalty averaged over $S$ (FREM) can leave dependence at sensitive values of small probability, and a penalty on the two groups of a discretized $S$ (MMD, sIPM-LFR) can leave dependence within each group; a downstream head can exploit what remains.

\Needspace{11\baselineskip}
\begin{wrapfigure}{r}{0.36\textwidth}
  \vspace{-13pt}
  \centering
  \includegraphics[width=0.34\textwidth]{main/fig5_profile_adult.pdf}
  \caption{Adult: $\widehat{\Delta}(s)$ of the worst-case heads $\hat{f}_1$ of supIPM-FRL and FREM at matched test accuracy.}
  \label{fig:profile-adult}
  \vspace{-10pt}
\end{wrapfigure}
supIPM-FRL instead constrains the discrepancy between $\mathbb{P}_{\mathbf{Z}\mid S=s}$ and $\mathbb{P}_{\mathbf{Z}}$ at every $s$ over all ReLU discriminators, so its fairness extends to every head in $\mathcal{F}_1$, in accordance with Corollary~\ref{cor:certificate}.  Figure~\ref{fig:profile-adult} shows this on Adult for the configurations with the smallest worst-case disparity among those whose test accuracy is within $0.004$ of the model trained without a penalty: the worst-case head of FREM attains a disparity many times larger than that of supIPM-FRL, whose disparity is small at every $s$.  Accordingly, the empirical supIPM on the test data in Appendix~\ref{app:real-measures} is smallest for supIPM-FRL on Adult and Crime at a matched level of prediction performance, although FREM attains a smaller value on ACSIncome.
