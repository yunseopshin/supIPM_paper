...
The ReLU class is also better suited to training with minibatches than the unit ball of a Gaussian RKHS used by FREM.  The MMD between two empirical distributions of $n$ representations requires the kernel at every pair of representations, that is, $O(n^2)$ kernel evaluations, and the closed-form penalty of FREM computed from the kernel sums and normalizations of a minibatch is not, in general, an unbiased estimate of its full-sample value.  In contrast, we show in Appendix~\ref{app:minibatch} that supIPM-FRL with the ReLU class admits minibatch learning.
...
We refer to this training method as supIPM-FRL, which can be implemented efficiently using minibatches (Appendix~\ref{app:minibatch}).  Further details and the algorithms are provided in  Appendices~\ref{app:ascent} and ~\ref{app:algorithm-steps}, respectively.
...
This appendix supplies the details of the learning procedure in Section~\ref{sec:supipm-objective}.  Appendix~\ref{app:ascent} describes the updates of  the sensitive values and the discriminators parameters together with the descent step for the encoder and the prediction head. Appendix~\ref{app:minibatch} writes the objective in Equation~(\ref{eq:supipm-penalized-objective}) in an equivalent form whose minibatch gradients are unbiased for fixed inner variables, and Appendix~\ref{app:algorithm-steps} summarizes the full-sample and the minibatch procedures in Algorithms~\ref{alg:supipm-frl} and~\ref{alg:supipm-frl-mb}.
...
\end{equation}
and write $\mathcal{J}:=\mathcal{J}_{[n]}$ for the full-sample objective.  Only the outer average is subsampled, and the factor $n$ and the weights $\widehat{w}_{\gamma}(i;s)$ retain their full-sample definitions.  Since $\Delta_{[n]}(s;\theta,\mu)$ is the signed sum inside the absolute value in Equation~(\ref{eq:discriminator-discrepancy}), we have $\Delta(s;\theta,\mu)=|\Delta_{[n]}(s;\theta,\mu)|$, and the identity $|x|=\max_{\chi\in\{-1,1\}}\chi x$ together with $\lambda\ge0$ gives
\begin{equation}
  \mathcal{L}(\phi,\psi)
  =\sup_{\boldsymbol{\vartheta}\in\Theta}\mathcal{J}(\phi,\psi;\boldsymbol{\vartheta}).
  \label{eq:mb-equivalent}
\end{equation}
Each sampled representation must be computed by the same encoder $h_{\phi}$ as in $\mathcal{J}$, so the output normalization of the encoder cannot use statistics of the batch.  The pointwise normalization of Appendix~\ref{app:ascent} satisfies this requirement.

\begin{proposition}[Unbiased minibatch objective and gradients]
\label{prop:minibatch}
...
\end{equation}
Both statements hold conditionally on all quantities computed before $\mathcal{B}$ is drawn, provided that $(\phi,\psi,\boldsymbol{\vartheta})$ is among these quantities and that $\mathcal{B}$ is drawn uniformly and independently of them.  Finally, suppose that $\boldsymbol{\vartheta}^{\star}$ maximizes $\mathcal{J}(\phi,\psi;\cdot)$ over $\Theta$ and is fixed before $\mathcal{B}$ is drawn, that $\mathcal{L}$ is differentiable at $(\phi,\psi)$, and that the condition above holds for $\phi$ and $\psi$ at $\boldsymbol{\vartheta}^{\star}$.  Then
\begin{equation}
...
\[
  ...
  =\mathcal{J}.
\]
The second identity in Equation~(\ref{eq:mb-unbiased-value}) then follows from Equation~(\ref{eq:mb-equivalent}).  The expectation $\mathbb{E}_{\mathcal{B}}$ is a finite average over the subsets of $[n]$ of size $|\mathcal{B}|$, so it commutes with differentiation, and the same calculation with $\nabla\mathcal{J}_{\{i\}}$ in place of $\mathcal{J}_{\{i\}}$ gives Equation~(\ref{eq:mb-unbiased-gradient}).  Conditioning on the quantities computed before $\mathcal{B}$ is drawn fixes $(\phi,\psi,\boldsymbol{\vartheta})$ and leaves $\mathbb{P}(i\in\mathcal{B})=|\mathcal{B}|/n$, which gives the conditional statements.  For the last assertion, the map $(\phi',\psi')\mapsto\mathcal{L}(\phi',\psi')-\mathcal{J}(\phi',\psi';\boldsymbol{\vartheta}^{\star})$ is nonnegative by Equation~(\ref{eq:mb-equivalent}) and vanishes at $(\phi,\psi)$, where it therefore attains its minimum.  Its gradients with respect to $\phi$ and to $\psi$ at $(\phi,\psi)$ therefore vanish, so $\nabla_{(\phi,\psi)}\mathcal{L}(\phi,\psi)=\nabla_{(\phi,\psi)}\mathcal{J}(\phi,\psi;\boldsymbol{\vartheta}^{\star})$, and Equation~(\ref{eq:mb-unbiased-gradient}) completes the proof.
\end{proof}

Proposition~\ref{prop:minibatch} gives unbiased stochastic gradients for the equivalent min--max objective, provided that the sensitive value, the discriminator and the sign used in a gradient step are fixed before its batch is drawn, for example from an earlier batch.  If these inner variables only approximately maximize $\mathcal{J}(\phi,\psi;\cdot)$, the gradient is unbiased for that of $\mathcal{J}$ at these variables, and Equation~(\ref{eq:mb-envelope-gradient}) requires in addition an exact maximizer.  Choosing them on the same batch targets a different quantity, since $\mathbb{E}_{\mathcal{B}}[\sup_{\boldsymbol{\vartheta}\in\Theta}\mathcal{J}_{\mathcal{B}}(\phi,\psi;\boldsymbol{\vartheta})]\ge\sup_{\boldsymbol{\vartheta}\in\Theta}\mathbb{E}_{\mathcal{B}}[\mathcal{J}_{\mathcal{B}}(\phi,\psi;\boldsymbol{\vartheta})]=\mathcal{L}(\phi,\psi)$ and the inequality can be strict.  Likewise, replacing $\widehat{q}_{\gamma}(s)$ by the average of $K_{\gamma}(S_j,s)$ over $j\in\mathcal{B}$ makes $\Delta_{\mathcal{B}}(s;\theta,\mu)$ a ratio of batch averages minus a batch average, for which the first identity in Equation~(\ref{eq:mb-unbiased-value}) fails in general.  The proposition holds for every batch size, but it bounds neither the optimization error nor the variance of the gradients, which can be large for small batches when the kernel weights concentrate on few observations.  Restricting $s$ to $\mathcal{S}_n$ and taking $\gamma=\gamma_n$ gives the empirical penalty $\widehat{D}_n(h_{\phi})$ of Section~\ref{sec:theory}, so Theorem~\ref{thm:uniform} and Corollary~\ref{cor:certificate} apply to this full-sample value for the learned encoder, which a maximum over finitely many candidates can underestimate.

\paragraph{Comparison with the MMD penalty.}  At each conditioning value, the penalty of FREM in Equation~(\ref{eq:eipm-estimator}) is the square root of a quadratic form in the kernel evaluated at pairs of representations \citep{kong2025fair}, so its value computed from the kernel sums and normalizations of a batch is not, in general, an unbiased estimate of the full-sample penalty.  Squared MMD admits unbiased estimators based on sampled pairs \citep{gretton2012kernel}, so this difficulty concerns the closed-form penalty of FREM rather than minibatch learning with the MMD in general.  The formulation above instead preserves the empirical supIPM penalty itself.
...
Algorithm~\ref{alg:supipm-frl-mb} is the minibatch version of Algorithm~\ref{alg:supipm-frl} and follows the order of sampling required by Proposition~\ref{prop:minibatch}.  The compass search, the candidate $s^{\star}$ and the signs $\chi_m:=\operatorname{sign}\Delta_{\mathcal{B}}(s^{\star};\theta_m,\mu_m)$, with $\operatorname{sign}(0):=1$, are computed on one batch with $\Delta$ replaced by $|\Delta_{\mathcal{B}}|$, and each gradient step uses a new batch drawn uniformly and independently of the preceding ones.  The ascent step uses
\begin{equation}
  H_{\mathcal{B}}(\mathbf{v})
  :=\sum_{m=1}^{M}\chi_m\,\Delta_{\mathcal{B}}(s^{\star};\theta_m,\mu_m),
  \label{eq:mb-stacked-ascent}
\end{equation}
whose gradient is, by the argument of Proposition~\ref{prop:minibatch}, unbiased for that of the same sum with $\mathcal{B}$ replaced by $[n]$.  This full-sample sum equals $H(\mathbf{v})$ in Equation~(\ref{eq:stacked-objectives}) when each $\chi_m$ agrees with the sign of $\Delta_{[n]}(s^{\star};\theta_m,\mu_m)$, and its gradient equals $\nabla_{\mathbf{v}}H(\mathbf{v})$ when, in addition, each $\Delta_{[n]}(s^{\star};\theta_m,\mu_m)$ is nonzero.  The descent step uses $\mathcal{J}_{\mathcal{B}}$ of Equation~(\ref{eq:mb-objective}) at $(s^{\star},\theta_{m^{\star}},\mu_{m^{\star}},\chi_{m^{\star}})$, where $m^{\star}$ indexes the discriminator with the largest $|\Delta_{\mathcal{B}}(s^{\star};\theta_m,\mu_m)|$ on the preceding batch.

\begin{algorithm}[H]
\caption{supIPM-FRL with minibatches}
\label{alg:supipm-frl-mb}
\begin{algorithmic}[1]
\REQUIRE the inputs of Algorithm~\ref{alg:supipm-frl} and a batch size $|\mathcal{B}|$
\STATE Initialize $\phi$, $\psi$, and $\mathbf{v}$ with $(\theta_m,\mu_m)\in\mathbb{S}^{d-1}\times[-1,1]$ for $m\in[M]$
\FOR{$r=1,\ldots,R$}
  \FOR{$j=1,\ldots,J_v$}
    \STATE Draw a batch $\mathcal{B}$ and run lines~\ref{alg:s-init}--\ref{alg:s-star} of Algorithm~\ref{alg:supipm-frl} with $\Delta$ replaced by $|\Delta_{\mathcal{B}}|$ \label{alg:mb-search}
    \STATE $\chi_m\leftarrow\operatorname{sign}\Delta_{\mathcal{B}}(s^{\star};\theta_m,\mu_m)$ for $m\in[M]$ \label{alg:mb-sign}
    \STATE Draw a new batch $\mathcal{B}$ and set $\mathbf{v}\leftarrow\Pi_{\mathbb{S}^{d-1}\times[-1,1]}\!\left(\mathbf{v}+\alpha_v\,\nabla_{\mathbf{v}}H_{\mathcal{B}}(\mathbf{v})\right)$
  \ENDFOR
...
% (one figure with panels for Adult, Crime and ACSIncome; prediction performance against the worst-case downstream $\operatorname{GDP}_{\infty}$ over $\mathcal{F}_1$; one curve per bandwidth)
