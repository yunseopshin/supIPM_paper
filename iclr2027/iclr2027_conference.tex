...
The penalty of FREM in Equation~(\ref{eq:eipm-estimator}) takes, at each conditioning value, the square root of a quadratic form in the kernel \citep{kong2025fair}, so its value computed on a mini-batch is, in general, a biased estimate of the batch penalty.  Unbiased estimators exist for the squared MMD \citep{gretton2012kernel}, which is a different penalty, whereas the formulation above preserves the empirical supIPM penalty.
