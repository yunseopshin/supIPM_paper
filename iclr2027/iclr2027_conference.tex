...
We jointly train the encoder $h_\phi$ and prediction head $f_\psi$ by minimizing the penalized supervised risk
...
Equation~(\ref{eq:supipm-penalized-objective}) gives a penalized formulation of empirical supervised risk minimization subject to $\widehat{\operatorname{supIPM}}^{\gamma}_{\mathcal{V}_{\mathrm{R}}}(\mathbf{Z};S)\le\rho$.
...
Consequently, representation obtained by FREM may retain dependence localized near $s_0$.
...
This appendix supplies the details of the learning procedure in Section~\ref{sec:supipm-objective}.  Appendix~\ref{app:ascent} describes the updates of  the sensitive values and the discriminators parameters together with the descent step for the encoder and the prediction head. 
Appendix~\ref{app:mini-batch} derives an equivalent formulation of Equation~(\ref{eq:supipm-penalized-objective}) for mini-batch training.
...
These steps yield an equivalent stochastic min--max formulation of Equation~(\ref{eq:supipm-penalized-objective}).
...
We compute the averages over the mini-batch.
...
Therefore, the normalization of encoder's output cannot use mini-batch statistics.
...
We fix the sensitive value, discriminator, and sign before drawing the mini-batch.
By Proposition~\ref{prop:mini-batch}, the resulting mini-batch gradient is unbiased for the gradient of $\mathcal{J}$.
Algorithm~\ref{alg:supipm-frl-mb} follows this order.
...
\end{equation}
whose gradient is, by the argument of Proposition~\ref{prop:mini-batch}, unbiased for that of the same sum with $\mathcal{B}$ replaced by $[n]$. 
Choose each $\chi_m$ to match the sign of $\Delta_{[n]}(s^{\star};\theta_m,\mu_m)$.
The batch sum then equals $H(\mathbf{v})$ in Equation~(\ref{eq:stacked-objectives}).
If all these discrepancies are nonzero, the gradient of the batch sum also equals $\nabla_{\mathbf{v}}H(\mathbf{v})$.
...
With a compactly supported kernel the representation collapses to a constant predictor at a smaller $\lambda$, so those curves stop at a disparity the Gaussian still reaches with a model that predicts.
