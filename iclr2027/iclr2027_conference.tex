...
Figure~\ref{fig:adult} shows the prediction-level disparity on Adult, and Figure~\ref{fig:worst-case} the worst-case downstream disparity over $\mathcal{F}_1$ on the three datasets; the prediction-level curves on Crime and ACSIncome and the worst-case curves over $\mathcal{F}_4$ and $\mathcal{F}_8$ are given in Appendix~\ref{app:real-tabular}.  At the prediction level, supIPM-FRL is comparable to the other methods.  Under the worst-case downstream disparity, supIPM-FRL attains a markedly smaller disparity than every other method at the same prediction performance on all three datasets, except at the lowest prediction performance on Crime, where the methods that use $S$ directly coincide with it.
