...
\paragraph{Measures.}  Prediction performance is the test accuracy for classification and the test $1-\mathrm{MAE}$ for regression, as in \citet{kong2025fair}.  Fairness is measured by two disparities.  The prediction-level disparity is $\operatorname{GDP}_{\infty}(f_{\psi}\circ h_{\phi})$ of Equation~(\ref{eq:linfty-gdp}), evaluated for the head trained jointly with the encoder.  It does not control a different head fitted later on the same representation, although such reuse is the purpose of FRL \citep{coston2021characterizing,jovanovic2023fare,langlade2025fairness,luo2026fair}.  We therefore also measure the largest disparity that a downstream head can attain.  For $c>0$, let $\mathcal{F}_c:=\{\sigma(\mathbf{w}^\top\mathbf{z}+b):\lVert\mathbf{w}\rVert_2\le c,\ b\in\mathbb{R}\}$ be the logistic heads with weight norm at most $c$, and define the worst-case downstream disparity
\begin{equation}
  \operatorname{GDP}_{\infty}(h;\mathcal{F}_c):=\sup_{f\in\mathcal{F}_c}\operatorname{GDP}_{\infty}(f\circ h).
  \label{eq:worst-case-gdp}
\end{equation}
Every head in $\mathcal{F}_c$ has a bounded H\"older norm, so Corollary~\ref{cor:certificate} bounds Equation~(\ref{eq:worst-case-gdp}) in terms of the empirical supIPM.  For a head $f$ we write $\Delta(s):=|\mathbb{E}[f(\mathbf{Z})\mid S=s]-\mathbb{E}[f(\mathbf{Z})]|$, so that $\operatorname{GDP}_{\infty}(f\circ h)=\sup_{s}\Delta(s)$.  All disparities are reported as kernel estimates on the test data; $\widehat{\Delta}(s)$ and $\widehat{\operatorname{GDP}}_{\infty}$ denote these estimates and are defined in Appendix~\ref{app:worst-case}.

\paragraph{Estimating the worst-case disparity.}  The supremum in Equation~(\ref{eq:worst-case-gdp}) is over infinitely many heads, so we search for the head that attains it.  The encoder is held fixed, and $\widehat{\operatorname{GDP}}_{\infty}$ and the empirical supervised risk $\widehat{\mathcal{R}}$ are computed on the training data.  For $\lambda_h>0$ we find
\begin{equation}
...
\end{equation}
by projected gradient descent over $(\mathbf{w},b)$.  The objective is the supervised risk minus $\lambda_h$ times the disparity.  With $\lambda_h=10^{3}$ the disparity dominates, so $\hat{f}_c$ is close to the head attaining the supremum.  The reported value is the disparity of this head on the test data, $\widehat{\operatorname{GDP}}_{\infty}(h;\mathcal{F}_c):=\widehat{\operatorname{GDP}}_{\infty}(\hat{f}_c\circ h)$.  The optimization is described in Appendix~\ref{app:worst-case}.
...
We compare supIPM-FRL with FREM over a grid of penalty parameters, using the worst-case downstream disparity over $\mathcal{F}_1$ as the fairness measure.  Each curve joins the configurations in increasing order of the penalty parameter.
...
The circled points are the configurations with the smallest disparity that retain the prediction performance of the model trained without a penalty.  At these points FREM still exhibits a peak in $\widehat{\Delta}(s)$ at $s_0$, whereas the $\widehat{\Delta}(s)$ of supIPM-FRL at $s_0$ is markedly smaller and not distinguishable from its values elsewhere. 
...
\paragraph{Baselines.}  We compare with seven methods.  Four of them use the continuous $S$ directly.  FREM \citep{kong2025fair} penalizes the EIPM of the representation.  Reg-GDP \citep{jiang2022generalized} and Reg-HGR \citep{mary2019fairness} penalize an estimate of the GDP of Equation~(\ref{eq:gdp}) and of the HGR maximal correlation of the prediction.  ADV \citep{zhang2018mitigating,kong2025fair} trains an adversary that predicts $S$ from the representation.  The other three, LAFTR \citep{madras2018learning}, MMD \citep{deka2023mmd} and sIPM-LFR \citep{kim2022learning}, are designed for a binary sensitive attribute: they are trained with $S$ discretized at its median and align the representations of the two groups.  All methods share the architecture of Appendix~\ref{app:real-settings}, are trained over a grid of penalty parameters with five random seeds, and are evaluated with the continuous $S$.
...
\paragraph{Results.}  Each point in the figures is the mean over the five seeds with bars of $\pm1$ standard deviation, and each curve traces, for one method, the configurations with the smallest disparity at each level of prediction performance, so that the upper left is favorable.  Figure~\ref{fig:adult} shows the prediction-level disparity on Adult, and Figure~\ref{fig:worst-case} the worst-case downstream disparity over $\mathcal{F}_1$ on the three datasets.  Appendix~\ref{app:real-tabular} gives the prediction-level curves on Crime and ACSIncome, which show the same pattern, and the worst-case curves over $\mathcal{F}_4$ and $\mathcal{F}_8$.  At the prediction level, the methods differ little: supIPM-FRL is close to FREM and Reg-GDP over the accuracies it attains, and the methods that discretize $S$ lie below or close to its curve.  Under the worst-case downstream disparity, the methods separate.  supIPM-FRL attains a markedly smaller disparity than every other method at the same prediction performance, on all three datasets and against both kinds of baselines; the only exception is the lowest prediction performance on Crime, where the methods that use $S$ directly coincide with it.
...
The two measures differ because they ask different questions.  The prediction-level disparity concerns the trained head only, whereas the worst-case disparity concerns every head in $\mathcal{F}_1$.  A penalty on the prediction (Reg-GDP, Reg-HGR) or on a single adversary (ADV, LAFTR) constrains only the dependence on $S$ that this head or adversary detects; a downstream head can exploit the rest.  A penalty averaged over $S$ (FREM) can leave dependence at sensitive values of small probability, and a penalty on the two groups of a discretized $S$ (MMD, sIPM-LFR) can leave dependence within each group.  supIPM-FRL instead constrains the discrepancy between $\mathbb{P}_{\mathbf{Z}\mid S=s}$ and $\mathbb{P}_{\mathbf{Z}}$ at every $s$ over all ReLU discriminators.  Its fairness therefore extends to every head in $\mathcal{F}_1$, in accordance with Corollary~\ref{cor:certificate}.  The empirical supIPM on the test data in Appendix~\ref{app:real-measures} supports this explanation: at a matched level of prediction performance it is smallest for supIPM-FRL on Adult and Crime, although FREM attains a smaller value on ACSIncome.
