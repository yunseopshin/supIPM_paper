...
To examine further the comparison of Appendix~\ref{ipm-class-real}, we also compare supFR, $\mathrm{supFR}_{\mathrm{MMD}}$ and FREM on the simulation of Section~\ref{sec:exp-simulation}, with the settings of Appendix~\ref{app:synth-experiments} unchanged.  Figure~\ref{fig:app-ablation-ipm-class-sim} shows the prediction performance against the worst-case downstream disparity over $\mathcal{F}_1$ and $\widehat{\Delta}_{\hat{f}_1}(s)$ of the circled models, as in Figure~\ref{fig:simulation}.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.78\textwidth]{appendix/fig_sim_supfr_mmd.pdf}
  \caption{Left: prediction performance against $\widehat{\operatorname{GDP}}_{\infty}(h_{\phi};\mathcal{F}_1)$ on the simulation data for supFR, FREM and $\mathrm{supFR}_{\mathrm{MMD}}$.  Right: $\widehat{\Delta}_{\hat{f}_1}(s)$ for the three models circled in the left panel.}
  \label{fig:app-ablation-ipm-class-sim}
\end{figure}

\paragraph{Results.}  supFR attains the lowest worst-case downstream disparity in the left panel and the smallest $\widehat{\Delta}_{\hat{f}_1}(s)$ at every $s$ in the right panel.  $\mathrm{supFR}_{\mathrm{MMD}}$ and FREM almost coincide away from $s_0$, whereas at $s_0$ the peak of $\mathrm{supFR}_{\mathrm{MMD}}$ is about half of that of FREM.  Since the two share the discriminator class $\mathcal{V}$ and differ only in taking the supremum instead of the average over $s$, this difference is due to the supremum: for every $f\in\mathcal{V}$ and $s\in\mathcal{S}$,
\begin{equation*}
  \Delta_{f}(s)
...
\end{equation*}
whereas the EIPM in (\ref{eq:eipm}) bounds only the average of $\Delta_f(S)$ over $\mathbb{P}_S$, to which the interval of small $\mathbb{P}_S$-probability around $s_0$ contributes little.  Thus the ReLU discriminator class determines the worst-case downstream disparity over the whole sensitive domain, as in Appendix~\ref{ipm-class-real}, and the supremum over $s$ additionally reduces the fairness violation at $s_0$.
