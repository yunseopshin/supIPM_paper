...
The proposed algorithm, however, can easily modified for high-dimensional continuous sensitive attributes. {\bf See Remark xx for discussions.}

An encoder $h:\mathcal{X}\to\mathcal{Z}$ maps $\mathbf{X}$ to a representation $\mathbf{Z}=h(\mathbf{X})$ in the representation space $\mathcal{Z}\subset \mathbb{R}^d.$ A prediction head $f:\mathcal{Z}\to\mathbb{R}$ maps the representation to a real-valued prediction, and $f\circ h$ denotes the composed prediction model
which maps an input to a prediction value. 
% When the encoder and the prediction head are parameterized, we write them as $h_{\phi}$ and $f_{\psi}$ with parameters $\phi$ and $\psi,$ respectively. 
...
The discrepancy between $\mathbb{P}_{\mathbf{Z}\mid S=s}$ and $\mathbb{P}_{\mathbf{Z}}$ is measured using the IPM
with an discriminator class $\mathcal{V}$, a class of real-valued discriminator functions on $\mathcal{Z}$. 
...
For a continuous sensitive attribute, \citet{kong2025fair} defines the expectation
...
\begin{equation}
  ...
  \label{eq:eipm}
\end{equation}
and proposes an FRL algorithm using EIPM (FREM).
...
It turns out that EIPM can controls fairness violation of $f \circ h$ only when $f$ belongs to the discriminator class $\mathcal{V}$.
...
To overcome this restriction, \citet{park2026relu} considers ReLU-IPM, an IPM whose discriminator class is indexed by a finite-dimensional parameter. ReLU-IPM controls the fairness violation of H\"older smooth prediction head.

Let $\mathbb{B}^d:=\{\mathbf{z}\in\mathbb{R}^d:\lVert \mathbf{z}\rVert_2\le 1\}$ be the unit ball and let $\mathbb{S}^{d-1}:=\{\theta\in\mathbb{R}^d:\lVert\theta\rVert_2=1\}$ be the unit sphere. \citet{park2026relu} defines the ReLU discriminator class $\mathcal{V}_{\mathrm{R}}$ on $\mathbb{B}^d$ by
\begin{equation}
...
\citet{park2026relu} computes ReLU-IPM between two distributions by applying a gradient ascent over $(\theta,\mu)$,  and projecting $\theta$ back onto the unit sphere in each update.
...
\end{equation}
This results implies that we can control H\"older-IPM by controlling ReLU-IPM, and thus control fairness violation of H\"older smooth prediction heads.
...
%we assume the following condition on the distribution of $S$ throughout this paper.
%\begin{assumption}[Positive density]
%\label{as:positive-density}
%$\mathbb{P}_S$ has a density $p_S$ with respect to the Lebesgue measure on $\mathcal{S}$, and $0<c_S\le p_S(s)\le C_S<\infty$ for every $s\in\mathcal{S}$.
%\end{assumption}
...
For a bounded measurable function $g:\mathcal{X}\rightarrow \mathbb{R},$ \citet{jiang2022generalized} defines generalized demographic parity (GDP) for a continuous sensitive attribute as
\begin{equation}
  \operatorname{GDP}(g)
  :=\mathbb{E}_{S}\!\left[
    \left|
  ...
  \label{eq:gdp}
\end{equation}
For $g=f\circ h$, $\operatorname{GDP}(g)$ averages $\left|\mathbb{E}[f(\mathbf{Z})\mid S]-\mathbb{E}[f(\mathbf{Z})]\right|$ over $S$.  Replacing the average by the supremum, as in supIPM, we define supGDP ($\operatorname{GDP}_{\infty}$) as
\begin{equation}
...
is not easy because $\mathcal{H}^{\beta}_d$ is not easily parameterizable.
To resolve this problem, we use ReLU-IPM. Since the discriminator class of ReLU-IPM has finite parameters,
computation of $\operatorname{supIPM}_{\mathcal{H}^{\beta}_d}(\mathbf{Z};S)$ can be done without much hamper.
Moreover, the inequality (\ref{eq:relu-holder}) implies
that $\operatorname{GDP}_{\infty}(f\circ h)$ is bounded by $\operatorname{supIPM}_{\mathcal{V}_R}(\mathbf{Z};S)$
...
The ReLU-IPM is better then MMD from computational point of view.  
Computing EIPM with MMD requires $O(n^3)$ evaluations at each update.
In contrast, supReLU-IPM only needs $O(n)$ computation at each update.
Moreover, a mini-batch learning is not possible for EIPM with MMD \citep{lee2026fair}, and thus  
$O(n^2)$ memories are required. In contrast, we show in Appendix~\ref{app:mini-batch} that supIPM-FRL with the ReLU-IPM admits a mini-batch learning.
...
Similar to EIPM, we can estimate supIPM with the ReLU discriminator class by kernel techniques. Let
\begin{equation}
...
\end{equation}
where $K_{\gamma}$ is a given kernel.  We propose to estimate supIPM with the ReLU discriminator class by
...
\end{align}
We prove in Theorem xx of Section xx that this estimator converges to the population supIPM
with a properly chose kernel and bandwidth.
...
The empirical supIPM in (\ref{eq:empirical-supipm}) is a supremum over the sensitive value $s$ and over the discriminator parameter $(\theta,\mu)$.
...
We alternate between gradient descent on $\mathcal{L}(\phi,\psi)$ and approximating the supremum in Equation~\ref{eq:supipm-penalized-objective} with $(\phi,\psi)$ fixed. 
...
ReLU-IPM and sigmoid-IPM achieve comparable worst-case disparity.
Their relative performance varies with the dataset and penalty strength.
Both achieve lower worst-case disparity than MMD at comparable prediction performance.
