...
\ref{ipm-class-real}의 결과를 further 분석하기 위해서 simulation study에서도 supFR, $\mathrm{supFR}_{\mathrm{MMD}}$ 그리고 FREM을 비교했다. 
(simulation 세팅과 같음을 설명)

\begin{figure}[H]
  \centering
  \includegraphics[width=0.78\textwidth]{appendix/fig_sim_supfr_mmd.pdf}
  \caption{Left: test accuracy against $\widehat{\operatorname{GDP}}_{\infty}(h_{\phi};\mathcal{F}_1)$ on simulation data for supFR, FREM and $\mathrm{supFR}_{\mathrm{MMD}}$.  Right: $\widehat{\Delta}_{\hat{f}_1}(s)$ for the three models circled in the left panel, chosen at matched test accuracy.}
  \label{fig:app-ablation-ipm-class-Delta}
\end{figure}

\paragraph{Results} supFR이 전반적으로 disparity가 항상 제일 낮고, $\mathrm{supFR}_{\mathrm{MMD}}$과 FREM은 전반적으로 비슷하게 나오나, $s_0$에서 차이가 난다고 설명. sup을 control하기에 더 sup(disparity)(수식으로 설명)이 더 낮게 나올수있음을 설명한다.
이를 통해서 worst-case prediction head에서 Relu가 큰영향을 끼치면서도, sup역시 $s_0$에서 fairness violation을 완와함을 설명함.
