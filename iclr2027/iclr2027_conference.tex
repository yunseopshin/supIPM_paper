...
\begin{align}
  ...
  \label{eq:supipm}
\end{align}
By definition, the constraint $\operatorname{supIPM}_{\mathcal{V}}(\mathbf{Z};S)\le\rho$ for a fairness level $\rho\ge0$ bounds the discrepancy $\operatorname{IPM}_{\mathcal{V}}(\mathbb{P}_{\mathbf{Z}\mid S=s},\mathbb{P}_{\mathbf{Z}})$ by $\rho$ at every $s\in\mathcal{S}$, and hence $\left|\mathbb{E}[v(\mathbf{Z})\mid S=s]-\mathbb{E}[v(\mathbf{Z})]\right|$ by the same $\rho$ for every discriminator $v\in\mathcal{V}$.  Because an average never exceeds a supremum, $\operatorname{EIPM}_{\mathcal{V}}(\mathbf{Z};S)\le\operatorname{supIPM}_{\mathcal{V}}(\mathbf{Z};S)$, so a representation with a small supIPM has a small EIPM as well.
% The converse fails without further regularity in $s$.

A prediction head $f$ is also a function of the representation, and if $f\in\mathcal{V}$, the difference between the conditional and the marginal means of its prediction $f(\mathbf{Z})$ is at most $\operatorname{supIPM}_{\mathcal{V}}(\mathbf{Z};S)$ at every $s\in\mathcal{S}$, even in a region of small $\mathbb{P}_S$-probability.  For a bounded measurable $g$, \citet{jiang2022generalized} define generalized demographic parity (GDP) for a continuous sensitive attribute as
\begin{equation}
  \operatorname{GDP}(g)
  :=\mathbb{E}_{S}\!\left[
    \left|
  ...
  \label{eq:gdp}
\end{equation}
For $g=f\circ h$, Equation~(\ref{eq:gdp}) averages this difference over $S$.  Replacing the average by a supremum, as in supIPM, we define $L_{\infty}$-generalized demographic parity ($\operatorname{GDP}_{\infty}$)
\begin{equation}
  ...
  \label{eq:linfty-gdp}
\end{equation}
For $f\in\mathcal{V}$, we thus obtain $\operatorname{GDP}_{\infty}(f\circ h)\le\operatorname{supIPM}_{\mathcal{V}}(\mathbf{Z};S)$, so supIPM, defined before a prediction head is chosen, controls $\operatorname{GDP}_{\infty}(f\circ h)$ for all such heads simultaneously.
