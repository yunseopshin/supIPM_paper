...
Proposition~\ref{prop:mini-batch} gives unbiased stochastic gradients for the equivalent min--max objective, provided that the sensitive value, the discriminator and the sign used in a gradient step are fixed before its mini-batch is drawn, for example from an earlier mini-batch.  If these inner variables only approximately maximize $\mathcal{J}(\phi,\psi;\cdot)$, the gradient is unbiased for that of $\mathcal{J}$ at these variables, and Equation~(\ref{eq:mb-envelope-gradient}) requires in addition an exact maximizer.  Choosing them on the same mini-batch targets a different quantity, since $\mathbb{E}_{\mathcal{B}}[\sup_{\boldsymbol{\vartheta}\in\Theta}\mathcal{J}_{\mathcal{B}}(\phi,\psi;\boldsymbol{\vartheta})]\ge\sup_{\boldsymbol{\vartheta}\in\Theta}\mathbb{E}_{\mathcal{B}}[\mathcal{J}_{\mathcal{B}}(\phi,\psi;\boldsymbol{\vartheta})]=\mathcal{L}(\phi,\psi)$ and the inequality can be strict.  Likewise, replacing $\widehat{q}_{\gamma}(s)$ by the average of $K_{\gamma}(S_j,s)$ over $j\in\mathcal{B}$ makes $\Delta_{\mathcal{B}}(s;\theta,\mu)$ a ratio of mini-batch averages minus a mini-batch average, for which the first identity in Equation~(\ref{eq:mb-unbiased-value}) fails in general.  The proposition holds for every mini-batch size, but it bounds neither the optimization error nor the variance of the gradients, which can be large for small mini-batches when the kernel weights concentrate on few observations.  Restricting $s$ to $\mathcal{S}_n$ and taking $\gamma=\gamma_n$ gives the empirical penalty $\widehat{D}_n(h_{\phi})$ of Section~\ref{sec:theory}, so Theorem~\ref{thm:uniform} and Corollary~\ref{cor:certificate} apply to this batch value for the learned encoder, which a maximum over finitely many candidates can underestimate.

\paragraph{Comparison with the MMD penalty.}  At each conditioning value, the penalty of FREM in Equation~(\ref{eq:eipm-estimator}) is the square root of a quadratic form in the kernel evaluated at pairs of representations \citep{kong2025fair}, so its value computed from the kernel sums and normalizations of a mini-batch is not, in general, an unbiased estimate of the batch penalty.  Squared MMD admits unbiased estimators based on sampled pairs \citep{gretton2012kernel}, so this difficulty concerns the closed-form penalty of FREM rather than mini-batch learning with the MMD in general.  The formulation above instead preserves the empirical supIPM penalty itself.
