...
\end{equation}
over $\lVert\mathbf{w}\rVert_2\le c$ and $b\in\mathbb{R}$, with $\lambda_h=10^{3}$ so that the fairness regularization term dominates.  The minimizer $\hat{f}_c$ approximates the head attaining the supremum in (\ref{eq:worst-case-gdp}), and the worst-case disparity is estimated by $\widehat{\operatorname{GDP}}_{\infty}(h_\phi;\mathcal{F}_c):=\widehat{\operatorname{GDP}}_{\infty}(\hat{f}_c\circ h_\phi)$.
...
We independently draw $S\sim\mathrm{Uniform}(0,1)$, $\mathbf{X}_{\mathrm{task}}\sim N(\mathbf{0},\mathbf{I}_4)$, $\mathbf{X}_{\mathrm{noise}}\sim N(\mathbf{0},\mathbf{I}_3)$ and $\varepsilon_1,\varepsilon_2\sim N(0,1)$, and set
\begin{equation*}
X_{\mathrm{loc}}:=3\exp\!\left\{-\frac{(S-s_0)^2}{2\tau^2}\right\}+0.5\,\varepsilon_1,
\qquad
Y:=\sigma\!\left(\boldsymbol{\upsilon}^\top\mathbf{X}_{\mathrm{task}}+X_{\mathrm{loc}}+0.3\,\varepsilon_2\right),
\end{equation*}
where $s_0=0.5$, $\tau=0.043$ and $\boldsymbol{\upsilon}\in\mathbb{R}^4$ is given.
Since $S$ affects $Y$ only near $s_0$, the worst-case fairness violation can occur near $s_0$, even though only a small proportion of observations fall in this narrow interval.
We compare supIPM-FRL with FREM by varying $\lambda$ in (\ref{eq:supipm-penalized-objective}) and (\ref{eq:frem-objective}), respectively, and measure fairness violation by the worst-case downstream disparity over $\mathcal{F}_1$.
...
\begin{tabular}{lll}
\toprule
Method & Type of sensitive attribute & Aim of algorithm \\
\midrule
supIPM-FRL & continuous & representation \\
FREM \citep{kong2025fair} & continuous & representation \\
Reg-GDP \citep{jiang2022generalized} & continuous or discrete & prediction  \\
Reg-HGR \citep{mary2019fairness} & continuous or discrete & prediction \\
ADV \citep{zhang2018mitigating} & continuous or discrete & prediction \\
LAFTR \citep{madras2018learning} & binary & representation \\
MMD \citep{deka2023mmd} & binary & representation \\
sIPM-LFR \citep{kim2022learning} & binary & representation \\
\bottomrule
\end{tabular}
\vspace{-6pt}
\end{table}
...
The baselines do not directly control disparity at every sensitive value, so worst-case fairness violations may arise with other downstream heads.
...
Figure~\ref{fig:profile-adult} shows $\widehat{\Delta}_{\hat{f}_1}(s)$ on Adult for supIPM-FRL and FREM at a test accuracy within $0.004$ of the model trained without a fairness penalty.
% For each method, we take the setting with the smallest worst-case disparity among these settings.
The disparity of supIPM-FRL is small over the whole sensitive domain, whereas that of FREM is substantially larger over most of the domain.
...
Future work includes extending supIPM to fairness criteria conditional on the outcome, such as equalized odds and equal opportunity.
