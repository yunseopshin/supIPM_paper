...
We then propose $\mathrm{FR}_{\sup}$, an algorithm for learning representations that control worst-case fairness violations in downstream predictions.
...
Experiments on tabular and graph benchmarks show that $\mathrm{FR}_{\sup}$ attains prediction performance comparable to existing methods while substantially reducing the worst-case fairness violation.
\end{abstract}
...
\begin{itemize}
  ...
  $\mathrm{FR}_{\sup}$, and prove that the worst-case fairness violations of a large class of prediction models constructed on the fair representation learned by $\mathrm{FR}_{\sup}$ are asymptotically controlled.
  \item We demonstrate empirically that $\mathrm{FR}_{\sup}$ controls fairness violations 
...
%Section~\ref{sec:supipm-objective} introduces $\mathrm{FR}_{\sup}$, which uses this empirical supIPM to learn representations with worst-case fairness control.
...
We refer to this learning algorithm as $\mathrm{FR}_{\sup}$, which can be implemented efficiently using mini-batches (Appendix~\ref{app:mini-batch}).  Further details and the algorithms are provided in  Appendices~\ref{app:ascent} and~\ref{app:algorithm-steps}, respectively.
...
Thus, for fixed $T$, the number of evaluations per iteration grows linearly with $r$, allowing $\mathrm{FR}_{\sup}$ to be extended to
...
% $\mathrm{FR}_{\sup}$ of Section~\ref{sec:supipm-objective} learns the encoder by penalizing the empirical supIPM, whereas the fairness of the learned representation is measured by the population supIPM.  
...
Moreover, the bound is uniform over $\mathcal{E}_n$, so it applies to the encoder learned by $\mathrm{FR}_{\sup}$.
...
\end{equation}
\end{corollary}
% This corollary shows that by learning an encoder $h$ with a small $\widehat{\operatorname{supIPM}}^{\gamma_n}_{\mathcal{V}_{\mathrm{R}},n}(h(\mathbf{X});S)$, $\mathrm{FR}_{\sup}$ controls the fairness violation $\operatorname{GDP}_{\infty,n}(f\circ h)$ uniformly over all prediction heads $f\in\mathcal{H}^{\beta}_d(B)$.
...
% This section evaluates $\mathrm{FR}_{\sup}$ on simulated data, where the location $s_0$ of the dependence on $S$ is known, and on three tabular datasets, where we compare it with existing baselines.
This section evaluates $\mathrm{FR}_{\sup}$ on simulated data and on three tabular datasets.
...
% We compare $\mathrm{FR}_{\sup}$ with FREM by varying the penalty parameter $\lambda$ of their training objectives, (\ref{eq:supipm-penalized-objective}) and (\ref{eq:frem-objective}) respectively, and measure fairness violation by the worst-case downstream disparity over $\mathcal{F}_1$.
...
To confirm this conjecture, we compare $\mathrm{FR}_{\sup}$ with FREM by varying $\lambda$ in (\ref{eq:frem-objective}) and (\ref{eq:supipm-penalized-objective}), respectively, and measure the worst-case downstream disparity over $\mathcal{F}_1$.
...
\paragraph{Results.}  In the left panel of Figure~\ref{fig:simulation}, $\mathrm{FR}_{\sup}$ attains a lower worst-case downstream disparity than FREM.
% Because $Y$ does not depend on $S$, the dependence near $s_0$ can be removed without giving up prediction performance, and the left panel shows that $\mathrm{FR}_{\sup}$ does remove it: its curve runs horizontally before it turns downward. The right panel shows what each penalty leaves behind.
...
% The right panel shows $\widehat{\Delta}_{\hat{f}_1}(s)$ of models at the circled points of left panel. For FREM, it has a peak at $s_0$, whereas for $\mathrm{FR}_{\sup}$, it is markedly smaller there.
...
For FREM, it has a peak at $s_0$, whereas for $\mathrm{FR}_{\sup}$, it is markedly smaller there.
...
% Thus, reducing the EIPM need not make the conditional and marginal distributions close at every sensitive value.
In contrast, $\mathrm{FR}_{\sup}$ penalizes the largest discrepancy over $s$ in (\ref{eq:supipm}), and thus controls such a worst-case fairness violation.
...
\paragraph{Baselines.}
We compare $\mathrm{FR}_{\sup}$ with seven baselines for fair representation learning (FRL) algorithms or for learning fair prediction models.
...
\midrule
$\mathrm{FR}_{\sup}$ & continuous & fair representation \\
...
$\mathrm{FR}_{\sup}$ is similar to the other methods at the prediction level, whereas its worst-case disparity is markedly smaller than that of every other method on all three datasets.
% Table~\ref{tab:matched-adult} reports the two disparities on Adult at test accuracy $0.838$--$0.846$, following Appendix~\ref{app:real-matched}. The worst-case disparity of $\mathrm{FR}_{\sup}$ is lower than that of every baseline.
...
% \toprule
%  & $\mathrm{FR}_{\sup}$ & FREM & Reg-GDP & Reg-HGR & ADV & LAFTR & MMD & sIPM-LFR \\
% \midrule
...
\begin{wrapfigure}{r}{0.36\textwidth}
  ...
  \vspace{-14pt}
  \caption{$\widehat{\Delta}_{\hat{f}_1}(s)$  of $\mathrm{FR}_{\sup}$ and FREM at matched test accuracy.}
...
In contrast, $\mathrm{FR}_{\sup}$ controls the largest ReLU-IPM between $\mathbb{P}_{\mathbf{Z}\mid S=s}$ and $\mathbb{P}_{\mathbf{Z}}$ over the sensitive domain, and Corollary~\ref{cor:certificate} implies that the empirical supIPM yields a bound on $\Delta_f(s)$ over $\mathcal{S}_n$ for every head $f$ of bounded H\"older norm.
Figure~\ref{fig:profile-adult} shows $\widehat{\Delta}_{\hat{f}_1}(s)$ on Adult for $\mathrm{FR}_{\sup}$ and FREM 
...
The disparity of $\mathrm{FR}_{\sup}$ is small over the whole sensitive domain, whereas that of FREM is substantially larger for $s$
near the left boundary and $s \approx 0.52$. 
% The disparity of $\mathrm{FR}_{\sup}$ is small over the whole sensitive domain,
...
\end{remark}
  ...
  % on Adult for the configurations with the smallest worst-case disparity among those whose test accuracy is within $0.004$ of the model trained without a penalty. the worst-case head of FREM attains a disparity many times larger than that of $\mathrm{FR}_{\sup}$, whose disparity is small at every $s$.  Accordingly, the empirical supIPM on the test data in Appendix~\ref{app:real-measures} is smallest for $\mathrm{FR}_{\sup}$ on Adult and Crime at a matched level of prediction performance, although FREM attains a smaller value on ACSIncome.

\subsection{Additional results and ablation studies}
We compare the methods under five further fairness measures and on the graph datasets in Appendix~\ref{app:real-additional}, and the computational cost of $\mathrm{FR}_{\sup}$ and FREM in Appendix~\ref{app:real-cost}. Appendix~\ref{app:ablation-ipm-class} examines the effect of the discriminator class $\mathcal{V}$, and Appendix~\ref{app:ablation-studies} the effects of the kernel $K$ in (\ref{eq:supipm-estimators}) and of the number $T$ of initial values of $s$.
...
% We proposed $\operatorname{GDP}_{\infty}$ and supIPM, which replace the average by a supremum over the sensitive domain and thereby impose one fairness level at every sensitive value.  $\mathrm{FR}_{\sup}$ estimates the supIPM from kernel weights and ReLU discriminators and searches the sensitive domain for the largest discrepancy.  We proved that the empirical supIPM converges uniformly to its population counterpart at the rate of nonparametric regression, so that the fairness level attained on the sample transfers to the population, and that this single quantity bounds the $\operatorname{GDP}_{\infty}$ of every prediction head of bounded H\"older norm.  
% On simulated and real data, $\mathrm{FR}_{\sup}$ achieves prediction performance and prediction-level disparity comparable to those of existing methods, while substantially reducing worst-case disparity over downstream heads.
...
We proposed supIPM, which measures the worst-case fairness violation of a representation for a continuous sensitive attribute. We then proposed $\mathrm{FR}_{\sup}$, which uses the empirical supIPM as a fairness penalty.
...
% With the ReLU discriminator class, $\mathrm{FR}_{\sup}$ admits a mini-batch formulation with unbiased gradients for fixed inner variables.
On simulated and real data, $\mathrm{FR}_{\sup}$ attains a markedly smaller worst-case downstream disparity than existing methods at comparable prediction performance.
...
% Future work includes extending $\mathrm{FR}_{\sup}$ to multivariate sensitive attributes.
...
\raggedbottom
\section{Details of Training \texorpdfstring{$\mathrm{FR}_{\sup}$}{FR-sup}}
\label{app:algorithm}
...
\begin{algorithm}[H]
\caption{$\mathrm{FR}_{\sup}$ with mini-batches}
...
The function $\omega$ is a Gaussian kernel centered at $s_0=0.5$.  Since $S$ is uniform on $[0,1]$, the interval $[s_0-\tau,\,s_0+\tau]$ has probability $2\tau=0.086$, so the dependence of $X_{\mathrm{loc}}$ on $S$ introduced through $\omega$ is concentrated in a small part of the sensitive domain.  Let $\boldsymbol{\upsilon}:=(0.8,-0.6,0.4,0.5)^\top$ and let $\sigma$ be the logistic function of Section~\ref{sec:experiments}.  The outcome and the input are
\begin{equation}
\begin{aligned}
  Y &:= \sigma\!\left(\boldsymbol{\upsilon}^\top\mathbf{X}_{\mathrm{task}}+X_{\mathrm{loc}} + 0.3\,\varepsilon_2\right), \\
...
This appendix supplements Section~\ref{sec:exp-real}.  Appendices~\ref{app:real-datasets}--\ref{app:worst-case} describe the datasets, the training settings, the baselines and the evaluation measures.  Appendix~\ref{app:real-additional} gives the results omitted from the main text, Appendix~\ref{app:real-cost} compares the computational cost of $\mathrm{FR}_{\sup}$ and FREM, and Appendix~\ref{app:ablation-ipm-class} examines the effect of the discriminator class.
...
Table~\ref{tab:baselines} in Section~\ref{sec:exp-real} classifies the baselines by the sensitive attribute and the learning target of their original formulation.  The baselines share the encoder, the prediction head and the training settings of Appendix~\ref{app:real-settings}, and differ from $\mathrm{FR}_{\sup}$ in their fairness term and in the range of its penalty parameter, as follows.
\begin{itemize}
...
Figure~\ref{fig:app-prediction-level} compares the methods by the prediction-level disparity on the three tabular datasets, with the binary-$S$ methods under both splits of $S$.  On Crime and ACSIncome, $\mathrm{FR}_{\sup}$, FREM and Reg-GDP have nearly the same curve, and the binary-$S$ methods are close to them.

\begin{figure}[H]
  \centering
  \includegraphics[trim=0 23.2 0 3.5, clip, width=0.88\textwidth]{appendix/fig2_own_head_adult.pdf}\\[1pt]
  \includegraphics[trim=0 23.2 0 3.5, clip, width=0.88\textwidth]{appendix/fig2_own_head_crime.pdf}\\[1pt]
  \includegraphics[trim=0 3.5 0 3.5, clip, width=0.88\textwidth]{appendix/fig2_own_head_acs.pdf}
  \caption{Prediction performance against the prediction-level disparity $\widehat{\operatorname{GDP}}_{\infty}(f_{\psi}\circ h_{\phi})$.}
  \label{fig:app-prediction-level}
\end{figure}

\paragraph{Worst-case downstream disparity.}
Figure~\ref{fig:app-c1} compares the methods by the worst-case downstream disparity over $\mathcal{F}_1$ on the same datasets.  At comparable prediction performance, $\mathrm{FR}_{\sup}$ attains the lowest worst-case disparity on the three datasets.  The supremum over $s$ and over the discriminators constrains the representation at every sensitive value, whereas an average over $S$, or a penalty on the trained head, leaves a dependence on $S$ that a downstream head can exploit.

\paragraph{Larger norm bounds.}  Figures~\ref{fig:app-c4} and~\ref{fig:app-c8} evaluate the worst-case downstream disparity over $\mathcal{F}_4$ and $\mathcal{F}_8$, respectively; these larger norm bounds admit a broader class of downstream heads.  At comparable prediction performance, $\mathrm{FR}_{\sup}$ attains the lowest worst-case disparity on the three tabular datasets, and its margin over the baselines is larger than at $c=1$, so its advantage persists across the evaluated norm bounds.

\begin{figure}[H]
  \centering
  \includegraphics[trim=0 21.3 0 3.5, clip, width=0.88\textwidth]{appendix/figA2_worst_head_budgets_adult_c1.pdf}\\[1pt]
  \includegraphics[trim=0 21.3 0 3.5, clip, width=0.88\textwidth]{appendix/figA2_worst_head_budgets_crime_c1.pdf}\\[1pt]
  \includegraphics[trim=0 3.5 0 3.5, clip, width=0.88\textwidth]{appendix/figA2_worst_head_budgets_acs_c1.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[trim=0 21.3 0 3.5, clip, width=0.88\textwidth]{appendix/figA2_worst_head_budgets_adult_c4.pdf}\\[1pt]
  \includegraphics[trim=0 21.3 0 3.5, clip, width=0.88\textwidth]{appendix/figA2_worst_head_budgets_crime_c4.pdf}\\[1pt]
  \includegraphics[trim=0 3.5 0 3.5, clip, width=0.88\textwidth]{appendix/figA2_worst_head_budgets_acs_c4.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[trim=0 21.3 0 3.5, clip, width=0.88\textwidth]{appendix/figA2_worst_head_budgets_adult_c8.pdf}\\[1pt]
  \includegraphics[trim=0 21.3 0 3.5, clip, width=0.88\textwidth]{appendix/figA2_worst_head_budgets_crime_c8.pdf}\\[1pt]
  \includegraphics[trim=0 3.5 0 3.5, clip, width=0.88\textwidth]{appendix/figA2_worst_head_budgets_acs_c8.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[trim=0 31.0 0 2.4, clip, width=0.88\textwidth]{appendix/figA_worst_head_mlp2_adult.pdf}\\[1pt]
  \includegraphics[trim=0 31.0 0 2.7, clip, width=0.88\textwidth]{appendix/figA_worst_head_mlp2_crime.pdf}\\[1pt]
  \includegraphics[trim=0 5.5 0 2.7, clip, width=0.88\textwidth]{appendix/figA_worst_head_mlp2_acs.pdf}
...
Figures~\ref{fig:app-gdp}--\ref{fig:app-supipm} compare the methods by the kernel estimate of GDP, the binned estimate of GDP and the empirical supIPM, respectively, all defined in Appendix~\ref{app:worst-case}.  The two estimates of GDP evaluate the trained prediction head and average the disparity over $S$, whereas the empirical supIPM evaluates the representation on the test dataset.  Under the two averages, $\mathrm{FR}_{\sup}$ and FREM attain similar values, and the remaining methods are two to four times larger.  Under the empirical supIPM, $\mathrm{FR}_{\sup}$ is markedly lower than every other method on Adult and Crime.  This measure is the quantity that $\mathrm{FR}_{\sup}$ penalizes, and Theorem~\ref{thm:uniform} shows that it converges uniformly to the population supIPM of the representation.

\paragraph{Dependence of the fitted prediction on $S$.}
Figures~\ref{fig:app-hgr} and~\ref{fig:app-mi-y} compare the methods by the HGR maximal correlation and by the mutual information between the prediction of the trained head and $S$.  No method attains the smallest value on all three datasets, and $\mathrm{FR}_{\sup}$ is comparable to FREM under both measures.  These two measures depend on the trained head alone, so a method that penalizes that head can attain a small value while the representation still depends on $S$.

\begin{figure}[H]
  \centering
  \includegraphics[trim=0 18.2 0 3.5, clip, width=0.88\textwidth]{appendix/figA6_other_measures_adult_gdp_w_kernel.pdf}\\[1pt]
  \includegraphics[trim=0 18.2 0 3.5, clip, width=0.88\textwidth]{appendix/figA6_other_measures_crime_gdp_w_kernel.pdf}\\[1pt]
  \includegraphics[trim=0 3.5 0 3.5, clip, width=0.88\textwidth]{appendix/figA6_other_measures_acs_gdp_w_kernel.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[trim=0 18.2 0 3.5, clip, width=0.88\textwidth]{appendix/figA6_other_measures_adult_gdp_wo_kernel.pdf}\\[1pt]
  \includegraphics[trim=0 18.2 0 3.5, clip, width=0.88\textwidth]{appendix/figA6_other_measures_crime_gdp_wo_kernel.pdf}\\[1pt]
  \includegraphics[trim=0 3.5 0 3.5, clip, width=0.88\textwidth]{appendix/figA6_other_measures_acs_gdp_wo_kernel.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[trim=0 17.7 0 3.5, clip, width=0.88\textwidth]{appendix/figA6_other_measures_adult_sup_ipm.pdf}\\[1pt]
  \includegraphics[trim=0 17.7 0 3.5, clip, width=0.88\textwidth]{appendix/figA6_other_measures_crime_sup_ipm.pdf}\\[1pt]
  \includegraphics[trim=0 3.5 0 3.5, clip, width=0.88\textwidth]{appendix/figA6_other_measures_acs_sup_ipm.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[trim=0 19.0 0 3.5, clip, width=0.88\textwidth]{appendix/figA6_other_measures_adult_hgr.pdf}\\[1pt]
  \includegraphics[trim=0 19.0 0 3.5, clip, width=0.88\textwidth]{appendix/figA6_other_measures_crime_hgr.pdf}\\[1pt]
  \includegraphics[trim=0 3.5 0 3.5, clip, width=0.88\textwidth]{appendix/figA6_other_measures_acs_hgr.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[trim=0 20.7 0 3.5, clip, width=0.88\textwidth]{appendix/figA6_other_measures_adult_mi_y_s.pdf}\\[1pt]
  \includegraphics[trim=0 20.7 0 3.5, clip, width=0.88\textwidth]{appendix/figA6_other_measures_crime_mi_y_s.pdf}\\[1pt]
  \includegraphics[trim=0 3.5 0 3.5, clip, width=0.88\textwidth]{appendix/figA6_other_measures_acs_mi_y_s.pdf}
...
On Pokec-z and Pokec-n we compare $\mathrm{FR}_{\sup}$ with FREM, Reg-GDP, Reg-HGR, ADV and MMD; LAFTR and sIPM-LFR are evaluated only on the tabular datasets.  In Figures~\ref{fig:app-graph}--\ref{fig:app-graph-mi}, the left column is Pokec-z and the right column is Pokec-n.  In the top row of Figure~\ref{fig:app-graph}, $\mathrm{FR}_{\sup}$ attains a higher accuracy than FREM and Reg-GDP at a given prediction-level disparity on both graphs, and MMD and Reg-HGR reach a given disparity only at a lower accuracy.  In the bottom row, $\mathrm{FR}_{\sup}$ attains a smaller worst-case downstream disparity than every baseline at comparable accuracy on both graphs.  
...
\begin{figure}[H]
  \centering
  \includegraphics[trim=0 25.7 0 2.2, clip, width=0.72\textwidth]{appendix/figA7_graph_own_head.pdf}\\[1pt]
  \includegraphics[trim=0 3.3 0 2.2, clip, width=0.72\textwidth]{appendix/figA7_graph_worst_head.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[trim=0 23.2 0 2.2, clip, width=0.67\textwidth]{appendix/figA7_graph_hgr.pdf}\\[1pt]
  \includegraphics[trim=0 3.3 0 2.2, clip, width=0.67\textwidth]{appendix/figA7_graph_mi_y_s.pdf}
...
Appendix~\ref{app:mini-batch} shows that, for fixed inner variables, the mini-batch objective of $\mathrm{FR}_{\sup}$ and its gradient are unbiased for their counterparts on the whole training sample.  The MMD penalty of FREM is biased on mini-batches, so it should, in principle, be computed on the whole training sample. Table~\ref{tab:cost} compares the cost of one gradient step on Adult for $\mathrm{FR}_{\sup}$ with a mini-batch and for FREM and $\mathrm{FR}_{\sup\mathrm{MMD}}$ with a single batch of $32{,}561$ observations, where $\mathrm{FR}_{\sup\mathrm{MMD}}$ denotes the variant of $\mathrm{FR}_{\sup}$ in which the ReLU-IPM is replaced by the MMD.
...
\midrule
$\mathrm{FR}_{\sup}$ & mini-batch ($1{,}024$) & 0.086 & 33 \\
FREM       & batch ($32{,}561$) & \multicolumn{2}{c}{out of memory} \\
$\mathrm{FR}_{\sup\mathrm{MMD}}$ & batch ($32{,}561$) & \multicolumn{2}{c}{out of memory} \\
\bottomrule
\end{tabular}
\end{table}

The MMD evaluates the kernel at every pair of representations, so its cost grows quadratically with the number of observations, and on the whole training sample it exceeds the $24$ GB of the GPU.  FREM and $\mathrm{FR}_{\sup\mathrm{MMD}}$ are therefore trained with mini-batches, whose penalty is biased for the penalty on the whole sample, as noted in Appendix~\ref{app:mini-batch}.  Figure~\ref{fig:cost} reports the time and the peak memory per step as the mini-batch grows, and the cross marks the first size that exceeds the memory of the GPU.  FREM already needs $8.5$ GB at $1{,}024$ observations.  The memory of $\mathrm{FR}_{\sup}$ grows linearly and the whole training sample takes $129$ MB, while its time per step is almost the same as with the mini-batch.
...
The linear cost of $\mathrm{FR}_{\sup}$, together with the unbiased mini-batch gradients of Appendix~\ref{app:mini-batch}, is the main reason for the ReLU discriminators of Section~\ref{sec:discriminator-choice}.

\subsection{Effect of IPM class}
\label{app:ablation-ipm-class}

The discriminator class $\mathcal{V}$ determines which discrepancies between $\mathbb{P}_{\mathbf{Z}\mid S=s}$ and $\mathbb{P}_{\mathbf{Z}}$ the supremum in (\ref{eq:supipm}) detects, and hence which downstream heads the penalty controls.  To examine its effect on $\mathrm{FR}_{\sup}$, we replace the ReLU class $\mathcal{V}_{\mathrm{R}}$ of (\ref{eq:relu-class}) either by the sigmoid discriminators $\sigma(\theta^{\top}\mathbf{z}+\mu)$ of sIPM-LFR \citep{kim2022learning} or by the unit ball of the Gaussian RKHS, whose IPM is the MMD used by FREM \citep{kong2025fair}; the latter variant is $\mathrm{FR}_{\sup\mathrm{MMD}}$ of Appendix~\ref{app:real-cost}.  Both variants retain the supremum over $s$ in (\ref{eq:supipm}) and differ from $\mathrm{FR}_{\sup}$ only in the discriminator class, whereas sIPM-LFR and FREM do not take this supremum.  The architecture, the training settings of Appendix~\ref{app:real-settings} and the measures of Appendix~\ref{app:worst-case} are unchanged, and the penalty values are chosen separately for each discriminator class.
...
Figure~\ref{fig:app-ablation-ipm-class} compares the three classes on the tabular datasets; the columns are Adult, Crime and ACSIncome from left to right, the top row reports the prediction-level disparity and the bottom row the worst-case downstream disparity over $\mathcal{F}_1$.  Figure~\ref{fig:app-ablation-ipm-class-Delta} compares $\mathrm{FR}_{\sup}$, FREM and $\mathrm{FR}_{\sup\mathrm{MMD}}$ on Adult.  Its left panel shows the test accuracy against the worst-case downstream disparity over $\mathcal{F}_1$, and the circles mark the model of each method chosen at matched test accuracy as in Figure~\ref{fig:profile-adult}.  Its right panel shows $\widehat{\Delta}_{\hat{f}_1}(s)$ of the three circled models over the sensitive domain.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.78\textwidth]{appendix/figAB_disc.pdf}
  \caption{Prediction performance against the prediction-level disparity and the worst-case downstream disparity over $\mathcal{F}_1$ for $\mathrm{FR}_{\sup}$ trained with each discriminator class.}
...
\begin{figure}[H]
  ...
  \caption{Left: test accuracy against $\widehat{\operatorname{GDP}}_{\infty}(h_{\phi};\mathcal{F}_1)$ on Adult for $\mathrm{FR}_{\sup}$, FREM and $\mathrm{FR}_{\sup\mathrm{MMD}}$.  Right: $\widehat{\Delta}_{\hat{f}_1}(s)$ for the three models circled in the left panel, chosen at matched test accuracy.}
...
In the right panel of Figure~\ref{fig:app-ablation-ipm-class-Delta}, $\widehat{\Delta}_{\hat{f}_1}(s)$ of $\mathrm{FR}_{\sup}$ is small over the whole sensitive domain, whereas that of $\mathrm{FR}_{\sup\mathrm{MMD}}$ is substantial near the left boundary and around $s\approx0.5$, with a profile close to that of FREM.  Since $\mathrm{FR}_{\sup\mathrm{MMD}}$ shares the discriminator class with FREM and the supremum over $s$ with $\mathrm{FR}_{\sup}$, this comparison suggests that the advantage of $\mathrm{FR}_{\sup}$ over FREM in Figure~\ref{fig:profile-adult} is attributable largely to the ReLU discriminator class rather than to the replacement of the average by the supremum.  This observation is consistent with Section~\ref{sec:discriminator-choice}: the ReLU class controls the fairness violation of H\"older smooth prediction heads through (\ref{eq:relu-holder}), whereas the Gaussian RKHS unit ball contains only infinitely differentiable functions (Section~\ref{sec:relu-ipm}).  The comparison leaves the case for supIPM intact.  Computing the empirical EIPM with $\mathcal{V}_{\mathrm{R}}$ requires adversarial learning at each $S_i$ (Section~\ref{sec:relu-ipm}), whereas the empirical supIPM in (\ref{eq:empirical-supipm}) requires a single maximization over $(s,\theta,\mu)$ (Section~\ref{sec:supipm-objective}), so the supremum over $s$ renders the ReLU discriminator class computationally tractable for a continuous sensitive attribute.
...
This appendix examines two choices in $\mathrm{FR}_{\sup}$ on the tabular datasets: the kernel in the estimator of the distribution and the number $T$ of candidate sensitive values. 
...
%
% The bandwidth $\gamma$ of the kernel $K_{\gamma}$ in (\ref{eq:supipm-estimators}) sets the range of sensitive values over which the conditional distribution at $s$ is smoothed.  We train $\mathrm{FR}_{\sup}$ at $\gamma\in\{0.10,0.25,0.50\}\,\widehat{\sigma}_S$ in place of the bandwidth of Appendix~\ref{app:synth-settings}, and leave the evaluation smoother unchanged.
%
...
%   \caption{Prediction performance against the prediction-level disparity $\widehat{\operatorname{GDP}}_{\infty}(f_{\psi}\circ h_{\phi})$ (top) and the worst-case downstream disparity $\widehat{\operatorname{GDP}}_{\infty}(h_{\phi};\mathcal{F}_{1})$ (bottom) for $\mathrm{FR}_{\sup}$ trained at $\gamma\in\{0.10,0.25,0.50\}$, on Adult (left), Crime (middle) and ACSIncome (right).}
...
\begin{figure}[H]
  ...
  \caption{Prediction performance against the prediction-level disparity and the worst-case downstream disparity over $\mathcal{F}_1$ for $\mathrm{FR}_{\sup}$ trained with each smoothing kernel.}
...
The compass search over $s$ of Appendix~\ref{app:ascent} starts from $T$ candidate sensitive values.  We train $\mathrm{FR}_{\sup}$ with $T\in\{5,10,15,20,25,30\}$ in addition to $T=17$ of Table~\ref{tab:hyperparameters}, using the same random seeds and the same penalty values for every $T$.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.78\textwidth]{appendix/figAB_T.pdf}
  \caption{Prediction performance against the prediction-level disparity and the worst-case downstream disparity over $\mathcal{F}_1$ for $\mathrm{FR}_{\sup}$ trained at $T\in\{5,10,15,17,20,25,30\}$.}
