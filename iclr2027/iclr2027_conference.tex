...
The MMD evaluates the kernel at every pair of representations, so its memory grows quadratically and neither penalty fits the batch in the $24$ GB of the GPU.  In practice they are therefore trained with mini-batches, without the unbiasedness of Appendix~\ref{app:mini-batch}.  Figure~\ref{fig:cost} gives the same step as the mini-batch grows.  FREM already needs $8.5$ GB at $1{,}024$ observations, so these penalties can be trained only up to a mini-batch that fits in memory.  The memory of supIPM-FRL grows linearly and the batch fits in $129$ MB, while its time per step is dominated by the inner updates and varies little with the mini-batch.
