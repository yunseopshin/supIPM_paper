...
\begin{itemize}
  ...
  $\mathrm{FR}_{\sup}$, and prove that the worst-case fairness violations of a large class of prediction models constructed on the fair representation learned by supIPM-FRL are asymptotically controlled.
...
We refer to this learning algorithm as supIPM-FRL, which can be implemented efficiently using mini-batches (Appendix~\ref{app:mini-batch}).  Further details and the algorithms are provided in  Appendices~\ref{app:ascent} and~\ref{app:algorithm-steps}, respectively.
...
Thus, for fixed $T$, the number of evaluations per iteration grows linearly with $r$, allowing supIPM-FRL to be extended to
...
Moreover, the bound is uniform over $\mathcal{E}_n$, so it applies to the encoder learned by supIPM-FRL.
...
This section evaluates supIPM-FRL on simulated data and on three tabular datasets.
...
% We compare supIPM-FRL with FREM by varying the penalty parameter $\lambda$ of their training objectives, (\ref{eq:supipm-penalized-objective}) and (\ref{eq:frem-objective}) respectively, and measure fairness violation by the worst-case downstream disparity over $\mathcal{F}_1$.
...
To confirm this conjecture, we compare supIPM-FRL with FREM by varying $\lambda$ in (\ref{eq:frem-objective}) and (\ref{eq:supipm-penalized-objective}), respectively, and measure the worst-case downstream disparity over $\mathcal{F}_1$.
...
\paragraph{Results.}  In the left panel of Figure~\ref{fig:simulation}, supIPM-FRL attains a lower worst-case downstream disparity than FREM.
...
% The right panel shows $\widehat{\Delta}_{\hat{f}_1}(s)$ of models at the circled points of left panel. For FREM, it has a peak at $s_0$, whereas for supIPM-FRL, it is markedly smaller there.
...
For FREM, it has a peak at $s_0$, whereas for supIPM-FRL, it is markedly smaller there.
...
% Thus, reducing the EIPM need not make the conditional and marginal distributions close at every sensitive value.
In contrast, supIPM-FRL penalizes the largest discrepancy over $s$ in (\ref{eq:supipm}), and thus controls such a worst-case fairness violation.
...
\paragraph{Baselines.}
We compare supIPM-FRL with seven baselines for fair representation learning (FRL) algorithms or for learning fair prediction models.
...
\midrule
supIPM-FRL & continuous & fair representation \\
...
supIPM-FRL is similar to the other methods at the prediction level, whereas its worst-case disparity is markedly smaller than that of every other method on all three datasets.
% Table~\ref{tab:matched-adult} reports the two disparities on Adult at test accuracy $0.838$--$0.846$, following Appendix~\ref{app:real-matched}. The worst-case disparity of supIPM-FRL is lower than that of every baseline.
...
In contrast, supIPM-FRL controls the largest ReLU-IPM between $\mathbb{P}_{\mathbf{Z}\mid S=s}$ and $\mathbb{P}_{\mathbf{Z}}$ over the sensitive domain, and Corollary~\ref{cor:certificate} implies that the empirical supIPM yields a bound on $\Delta_f(s)$ over $\mathcal{S}_n$ for every head $f$ of bounded H\"older norm.
Figure~\ref{fig:profile-adult} shows $\widehat{\Delta}_{\hat{f}_1}(s)$ on Adult for supIPM-FRL and FREM 
...
The disparity of supIPM-FRL is small over the whole sensitive domain, whereas that of FREM is substantially larger for $s$
near the left boundary and $s \approx 0.52$. 
% The disparity of supIPM-FRL is small over the whole sensitive domain,
...
We compare the methods under five further fairness measures and on the graph datasets in Appendix~\ref{app:real-additional}, and the computational cost of supIPM-FRL and FREM in Appendix~\ref{app:real-cost}. Appendix~\ref{app:ablation-studies} examines (i) the effects of the kernel $K$ in (\ref{eq:supipm-estimators}), (ii) the discriminator class $\mathcal{V}$ and (iii) the number $T$ of initial values of $s$.
...
We proposed supIPM, which measures the worst-case fairness violation of a representation for a continuous sensitive attribute. We then proposed supIPM-FRL, which uses the empirical supIPM as a fairness penalty.
...
This appendix supplements Section~\ref{sec:exp-real}.  Appendices~\ref{app:real-datasets}--\ref{app:worst-case} describe the datasets, the training settings, the baselines and the evaluation measures.  Appendix~\ref{app:real-additional} gives the results omitted from the main text, and Appendix~\ref{app:real-cost} compares the computational cost of supIPM-FRL and FREM.
...
The penalty parameter $\lambda$ of (\ref{eq:supipm-penalized-objective}) takes values between $0.003$ and $3$ for supIPM-FRL.
...
Table~\ref{tab:baselines} in Section~\ref{sec:exp-real} classifies the baselines by the sensitive attribute and the learning target of their original formulation.  The baselines share the encoder, the prediction head and the training settings of Appendix~\ref{app:real-settings}, and differ from supIPM-FRL in their fairness term and in the range of its penalty parameter, as follows.
\begin{itemize}
...
Figure~\ref{fig:app-prediction-level} compares the methods by the prediction-level disparity on the three tabular datasets, with the binary-$S$ methods under both splits of $S$.  On Crime and ACSIncome, supIPM-FRL, FREM and Reg-GDP have nearly the same curve, and the binary-$S$ methods are close to them.
...
Figure~\ref{fig:app-c1} compares the methods by the worst-case downstream disparity over $\mathcal{F}_1$ on the same datasets.  At comparable prediction performance, supIPM-FRL attains the lowest worst-case disparity on the three datasets.  The supremum over $s$ and over the discriminators constrains the representation at every sensitive value, whereas an average over $S$, or a penalty on the trained head, leaves a dependence on $S$ that a downstream head can exploit.

\paragraph{Larger norm bounds.}  Figures~\ref{fig:app-c4} and~\ref{fig:app-c8} evaluate the worst-case downstream disparity over $\mathcal{F}_4$ and $\mathcal{F}_8$, respectively; these larger norm bounds admit a broader class of downstream heads.  At comparable prediction performance, supIPM-FRL attains the lowest worst-case disparity on the three tabular datasets, and its margin over the baselines is larger than at $c=1$, so its advantage persists across the evaluated norm bounds.
...
On Pokec-z and Pokec-n we compare supIPM-FRL with FREM, Reg-GDP, Reg-HGR, ADV and MMD; LAFTR and sIPM-LFR are evaluated only on the tabular datasets.  In Figures~\ref{fig:app-graph}--\ref{fig:app-graph-mi}, the left column is Pokec-z and the right column is Pokec-n.  In the top row of Figure~\ref{fig:app-graph}, supIPM-FRL attains a higher accuracy than FREM and Reg-GDP at a given prediction-level disparity on both graphs, and MMD and Reg-HGR reach a given disparity only at a lower accuracy.  In the bottom row, supIPM-FRL attains a smaller worst-case downstream disparity than every baseline at comparable accuracy on both graphs.  
...
Appendix~\ref{app:mini-batch} shows that, for fixed inner variables, the mini-batch objective of supIPM-FRL and its gradient are unbiased for their counterparts on the whole training sample.  The MMD penalty of FREM is biased on mini-batches, so it should, in principle, be computed on the whole training sample. Table~\ref{tab:cost} compares the cost of one gradient step on Adult for supIPM-FRL with a mini-batch and for FREM and supMMD-FRL with a single batch of $32{,}561$ observations, where supMMD-FRL replaces the ReLU-IPM in supIPM-FRL by the MMD.
...
The MMD evaluates the kernel at every pair of representations, so its cost grows quadratically with the number of observations, and on the whole training sample it exceeds the $24$ GB of the GPU.  FREM and supMMD-FRL are therefore trained with mini-batches, whose penalty is biased for the penalty on the whole sample, as noted in Appendix~\ref{app:mini-batch}.  Figure~\ref{fig:cost} reports the time and the peak memory per step as the mini-batch grows, and the cross marks the first size that exceeds the memory of the GPU.  FREM already needs $8.5$ GB at $1{,}024$ observations.  The memory of supIPM-FRL grows linearly and the whole training sample takes $129$ MB, while its time per step is almost the same as with the mini-batch.
...
이 subsection에서는 IPM class $\mathcal{V}$의 choice가 $\mathrm{FR}_{\sup}$에 주는 영향을 분석한다. 그러기 위해서 Relu-IPM 외에 sigoild ipm과 MMD를 추가로 고려한다. supIPM을 구하는데 있어서   replace the ReLU class $\mathcal{V}_{\mathrm{R}}$ of (\ref{eq:relu-class})를 the sigmoid discriminators $\sigma(\theta^{\top}\mathbf{z}+\mu)$ used by sIPM-LFR \citep{kim2022learning} and by the unit ball of the Gaussian RKHS, whose IPM is the MMD used by FREM 로 replacement해서 각 데이터에서 보고, Adult에서는 MMD를 사용한 $\mathrm{FR}_{\sup \mathrm{MMD}}$와 $\mathrm{FR}_{\sup \mathrm{MMD}}$의 worst-case disparity $\hat{\Delta}_{\hat{f}_1}(s)$를 matched test accuracy에서 비교한다.



The discriminator class $\mathcal{V}$ determines which discrepancies between $\mathbb{P}_{\mathbf{Z}\mid S=s}$ and $\mathbb{P}_{\mathbf{Z}}$ the supremum in (\ref{eq:supipm}) detects, and hence which downstream heads the penalty controls.  We replace the ReLU class $\mathcal{V}_{\mathrm{R}}$ of (\ref{eq:relu-class}) by the sigmoid discriminators $\sigma(\theta^{\top}\mathbf{z}+\mu)$ used by sIPM-LFR \citep{kim2022learning} and by the unit ball of the Gaussian RKHS, whose IPM is the MMD used by FREM \citep{kong2025fair}. 
...
\begin{figure}[H]
  ...
  \caption{Prediction performance against the prediction-level disparity and the worst-case downstream disparity over $\mathcal{F}_1$ for supIPM-FRL trained with each discriminator class.}
...
\begin{figure}[H]
  ...
  \caption{$\widehat{\Delta}_{\hat{f}_1}(s)$  of supIPM-FRL, FREM, $\mathrm{FR}_{\sup \mathrm{MMD}}$ at matched test accuracy.}
...
\paragraph{Results.}  In Figure~\ref{fig:app-ablation-ipm-class}, the three discriminator classes yield similar prediction-level curves.  The ReLU and the sigmoid discriminators attain comparable worst-case downstream disparities, and their ordering varies with the dataset and with $\lambda$.  Both attain a markedly lower worst-case disparity than the MMD at comparable prediction performance.

Figure ~\ref{fig:app-ablation-ipm-class-Delta}를 봤을때 ReLU-IPM을 사용한 $\mathrm{FR}_{\sup}$ 만 worst-case disparity가 낮고 MMD를 사용한 경우에는 크게 나왔다. 이를 봤을때 Adult에서 $\mathrm{FR}_{\sup}$가 FREM 대비 가지는 Figure5의 이점은 sup의 성질 그 자체보단 Discriminator가 더 큰 class의 prediction head를 포함할 수 있으므로, 얻어지는 이점이라 사료된다. 다만 이 결과가 supIPM의 정당성을 약화시키는건 아니다. 왜냐면 supIPM을 사용하기에 3.2에서 언급한 EIPM은 각 $S_i$에서 adversail learning을 해야되서 생기는 computation cost를 상당부분 완화할수 있기 때문이다.
...
This appendix examines three choices in supIPM-FRL on the tabular datasets: the kernel in the estimator of the distribution, the class of the discriminators and the number $T$ of candidate sensitive values. 
% Section~\ref{sec:exp-real} uses the Gaussian kernel, the ReLU class $\mathcal{V}_{\mathrm{R}}$ and $T=17$. 
Each ablation varies one choice and keeps the architecture and the training settings of Appendix~\ref{app:real-settings} and the measures of Appendix~\ref{app:worst-case} unchanged, except that the penalty values are chosen separately for each discriminator class in Appendix~\ref{app:ablation-ipm-class}.  In each figure the columns are Adult, Crime and ACSIncome from left to right, the top row reports the prediction-level disparity and the bottom row the worst-case downstream disparity over $\mathcal{F}_1$.
...
\begin{figure}[H]
  ...
  \caption{Prediction performance against the prediction-level disparity and the worst-case downstream disparity over $\mathcal{F}_1$ for supIPM-FRL trained with each smoothing kernel.}
...
The compass search over $s$ of Appendix~\ref{app:ascent} starts from $T$ candidate sensitive values.  We train supIPM-FRL with $T\in\{5,10,15,20,25,30\}$ in addition to $T=17$ of Table~\ref{tab:hyperparameters}, using the same random seeds and the same penalty values for every $T$.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.78\textwidth]{appendix/figAB_T.pdf}
  \caption{Prediction performance against the prediction-level disparity and the worst-case downstream disparity over $\mathcal{F}_1$ for supIPM-FRL trained at $T\in\{5,10,15,17,20,25,30\}$.}
