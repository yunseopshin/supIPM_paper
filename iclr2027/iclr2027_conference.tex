...
Fair representation learning (FRL) seeks a representation that is independent of a given sensitive attribute yet informative for downstream prediction tasks 
...
various integral probability metrics (IPMs) have been used to measure the degree of fairness of a given representation
...
an extension of these methods to FRL has been proposed by \citet{kong2025fair}. The core idea of such methods
is to control the average fairness level of a given model or representation with respect to 
a continuous sensitive attribute $S.$ 
A critical limitation, however, in controlling the average fairness level is that under-represented sensitive groups could be highly discriminated.
...
the worst-case fairness violation is well controlled by controlling supIPM. Moreover,
a uniformly consistent estimator of supIPM can be obtained by combining ReLU-IPM and kernel density estimation,  and
...
\begin{itemize}
  ...
  supIPM-FRL, and prove that the worst-case fairness levels of a wide class of prediction models constructed on the fair representation learned by supIPM-FRL are asymptotically controlled.
...
In general, the MMD computed on a mini-batch is a biased estimate of the MMD computed on the entire training sample \citep{lee2026fair}.  Training with the MMD therefore requires the entire sample.
...
Appendix~\ref{app:mini-batch} shows that supIPM-FRL admits mini-batch learning.  The MMD penalty of FREM carries no such guarantee \citep{lee2026fair}, so it should be trained on the batch, that is, on the whole training sample.  Table~\ref{tab:cost} reports one gradient step on Adult under these two regimes.  It also covers sup-MMD, the variant of Appendix~\ref{app:ablation-ipm-class} with the MMD in place of the ReLU discriminators.
