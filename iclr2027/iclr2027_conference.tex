...
over $\lVert\mathbf{w}\rVert_2\le c$ and $b\in\mathbb{R}$, with $\lambda_h=10^{3}$ so that the fairness regularization term dominates. Let $\hat{f}_c(\cdot):=f_{\hat{\mathbf{w}}_c,\hat{b}_c}(\cdot)$, where $(\hat{\mathbf{w}}_c,\hat{b}_c)$ minimizes $\widetilde{\mathcal{L}}(\mathbf{w},b)$ subject to $\lVert\mathbf{w}\rVert_2\le c$ and $b\in\mathbb{R}$.
  This head approximates the prediction head attaining the supremum in (\ref{eq:worst-case-gdp}), and the worst-case disparity is estimated by $\widehat{\operatorname{GDP}}_{\infty}(h_\phi;\mathcal{F}_c):=\widehat{\operatorname{GDP}}_{\infty}(\hat{f}_c\circ h_\phi)$.
...
To confirm this conjecture, we compare supFR with FREM by varying $\lambda$ in (\ref{eq:supipm-penalized-objective}) and  (\ref{eq:frem-objective}), respectively, and measure the worst-case downstream disparity over $\mathcal{F}_1$.
...
\paragraph{Results.}  In the left panel of Figure~\ref{fig:simulation}, supFR attains a lower worst-case downstream disparity than FREM.
...
Figure~\ref{fig:adult} compares the methods by the prediction-level disparity and Figure~\ref{fig:worst-case} by the worst-case downstream disparity over $\mathcal{F}_1$.
...
supFR is similar to the other methods at the prediction level, whereas its worst-case disparity is markedly smaller than that of every other method on all three datasets.
...
On simulated and real data, supFR attains a markedly smaller worst-case downstream disparity than existing methods at comparable prediction performance.
...
\paragraph{Prediction and fairness measures.}  All measures are computed on the test data, indexed here by $i=1,\ldots,n$.  Prediction performance is the accuracy for classification and $1-\mathrm{MAE}$ for regression.  Fairness is measured by the prediction-level disparity $\operatorname{GDP}_{\infty}(f_{\psi}\circ h_{\phi})$ and by the worst-case downstream disparity $\operatorname{GDP}_{\infty}(h_{\phi};\mathcal{F}_c)$ of (\ref{eq:worst-case-gdp}), with $c=1$ except in Figures~\ref{fig:app-c4} and~\ref{fig:app-c8}.  Both are computed from the estimates
\begin{equation}
...
\paragraph{Worst-case downstream disparity.}  The head $\hat{f}_c$ of (\ref{eq:worst-case-estimate}) is computed with $\widehat{\operatorname{GDP}}_{\infty}$ of (\ref{eq:gdp-estimate}) formed from the training sample, by projected gradient descent over $(\mathbf{w},b)$ from eight initial heads, with $\mathbf{w}$ projected onto the ball of radius $c$ after every step.  The reported value $\widehat{\operatorname{GDP}}_{\infty}(h;\mathcal{F}_c)=\widehat{\operatorname{GDP}}_{\infty}(\hat{f}_c\circ h)$ of Section~\ref{sec:experiments} is computed on the test sample.
...
\paragraph{Worst-case downstream disparity.}
Figure~\ref{fig:app-c1} compares the methods by the worst-case downstream disparity over $\mathcal{F}_1$ on the same datasets.  At comparable prediction performance, supFR attains the lowest worst-case disparity on the three datasets.  The supremum over $s$ and over the discriminators constrains the representation at every sensitive value, whereas an average over $S$, or a penalty on the trained head, leaves a dependence on $S$ that a downstream head can exploit.

\paragraph{Larger norm bounds.}  Figures~\ref{fig:app-c4} and~\ref{fig:app-c8} evaluate the worst-case downstream disparity over $\mathcal{F}_4$ and $\mathcal{F}_8$, respectively; these larger norm bounds admit a broader class of downstream heads.  At comparable prediction performance, supFR attains the lowest worst-case disparity on the three tabular datasets, and its margin over the baselines is larger than at $c=1$, so its advantage persists across the evaluated norm bounds.
...
\begin{figure}[H]
  ...
  \caption{Prediction performance against the worst-case downstream disparity $\widehat{\operatorname{GDP}}_{\infty}(h_{\phi};\mathcal{F}_{1})$.}
  \label{fig:app-c1}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Prediction performance against the worst-case downstream disparity $\widehat{\operatorname{GDP}}_{\infty}(h_{\phi};\mathcal{F}_{4})$.}
  \label{fig:app-c4}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Prediction performance against the worst-case downstream disparity $\widehat{\operatorname{GDP}}_{\infty}(h_{\phi};\mathcal{F}_{8})$.}
  \label{fig:app-c8}
\end{figure}

\paragraph{Matched prediction performance.}
\label{app:real-matched}
Table~\ref{tab:matched} compares the methods at a fixed level of prediction performance on each tabular dataset.  For each method and each disparity, the entry is the smallest disparity among the models of that method whose prediction performance lies within $0.004$ of the level.  A dash indicates that no model of that method meets this criterion, and the smallest value in each row is in bold.  supFR attains the smallest value in every row.  Under the worst-case disparity, its value is at least three times smaller than that of every baseline on the three datasets.

\begin{table}[H]
\centering
\caption{Prediction-level disparity $\widehat{\operatorname{GDP}}_{\infty}(f_{\psi}\circ h_{\phi})$ and worst-case downstream disparity $\widehat{\operatorname{GDP}}_{\infty}(h_{\phi};\mathcal{F}_1)$ on the tabular datasets at matched prediction performance.}
\label{tab:matched}
\footnotesize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{lrrrrrrrr}
...
\end{equation*}
which have the same Lipschitz bound as the heads of $\mathcal{F}_c$.  Figure~\ref{fig:app-mlp-head} reports the worst-case downstream disparity over $\mathcal{F}'_1$.  The heads have $16$ hidden units and are fitted as in Appendix~\ref{app:worst-case}, with $\mathbf{W}_1$ projected onto the spectral-norm ball and $\mathbf{w}_2$ onto the ball of radius $c$ after every step.  At comparable prediction performance, the values over $\mathcal{F}'_1$ are close to those over $\mathcal{F}_1$.
...
\begin{figure}[H]
  ...
  \caption{Prediction performance against the worst-case downstream disparity over the two-layer heads, $\widehat{\operatorname{GDP}}_{\infty}(h_{\phi};\mathcal{F}'_{1})$.}
...
\begin{figure}[H]
  ...
  \caption{Prediction performance against the prediction-level disparity $\widehat{\operatorname{GDP}}_{\infty}(f_{\psi}\circ h_{\phi})$ and the worst-case downstream disparity $\widehat{\operatorname{GDP}}_{\infty}(h_{\phi};\mathcal{F}_1)$ on Pokec-z and Pokec-n.}
  \label{fig:app-graph}
\end{figure}
...
Figure~\ref{fig:app-ablation-ipm-class} compares the three classes on the tabular datasets; the columns are Adult, Crime and ACSIncome from left to right, the top row reports the prediction-level disparity and the bottom row the worst-case downstream disparity over $\mathcal{F}_1$.
...
% The grid of penalty values is chosen separately for each class, since the three IPMs are not on a common scale, and the classes are compared at comparable prediction performance.
  ...
  Figure~\ref{fig:app-ablation-ipm-class-Delta} compares supFR, FREM and $\mathrm{supFR}_{\mathrm{MMD}}$ on Adult.  Its left panel shows the test accuracy against the worst-case downstream disparity over $\mathcal{F}_1$, and the circles mark the model of each method chosen at matched test accuracy as in Figure~\ref{fig:profile-adult}.  Its right panel shows $\widehat{\Delta}_{\hat{f}_1}(s)$ of the three circled models over the sensitive domain.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.78\textwidth]{appendix/figAB_disc.pdf}
  \caption{Prediction performance against the prediction-level disparity and the worst-case downstream disparity over $\mathcal{F}_1$ for supFR trained with each discriminator class.}
...
\paragraph{Results.}  In Figure~\ref{fig:app-ablation-ipm-class}, the three discriminator classes yield similar prediction-level curves.  The ReLU and the sigmoid discriminators attain comparable worst-case downstream disparities, and their ordering varies with the dataset and with $\lambda$.  Both attain a markedly lower worst-case disparity than the MMD at comparable prediction performance on the three datasets.
...
\paragraph{Results.}  At comparable prediction performance, supFR attains the lowest worst-case downstream disparity in the left panel, and its $\widehat{\Delta}_{\hat{f}_1}(s)$ is the smallest over the whole sensitive domain in the right panel.  $\widehat{\Delta}_{\hat{f}_1}(s)$ of $\mathrm{supFR}_{\mathrm{MMD}}$ has $\widehat{\Delta}_{\hat{f}_1}$ close to that of FREM away from $s_0$, whereas at $s_0$ its peak is about half of that of FREM.  Since the two share the discriminator class $\mathcal{V}$ and differ only in taking the supremum instead of the average over $s$, this difference is due to the supremum: for every $f\in\mathcal{V}$ and $s\in\mathcal{S}$,
\begin{equation*}
  \Delta_{f}(s)
...
\begin{figure}[H]
  ...
  \caption{Prediction performance against the prediction-level disparity and the worst-case downstream disparity over $\mathcal{F}_1$ for supFR trained with each smoothing kernel.}
...
\begin{figure}[H]
  ...
  \caption{Prediction performance against the prediction-level disparity and the worst-case downstream disparity over $\mathcal{F}_1$ for supFR trained at $T\in\{5,10,15,17,20,25,30\}$.}
