...
In general, the MMD computed on a mini-batch is a biased estimate of the MMD computed on the entire training sample.  Training with the MMD therefore requires the entire sample.
...
In contrast, supIPM-FRL controls the largest ReLU-IPM between $\mathbb{P}_{\mathbf{Z}\mid S=s}$ and $\mathbb{P}_{\mathbf{Z}}$ over the sensitive domain. Corollary~\ref{cor:certificate} shows that the empirical supIPM uniformly controls a downstream disparity over all sensitive values.
Figure~\ref{fig:profile-adult} shows this property on Adult. We consider settings with test accuracy within $0.004$ of the baseline accuracy at $\lambda=0$. For each method, we take the setting with the lowest worst-case disparity among them. At comparable test accuracy, supIPM-FRL keeps disparity low across the sensitive domain even for the worst-case head, whereas FREM exhibits substantially larger disparities.
...
We proposed supIPM to measure worst-case fairness violations of representations for continuous sensitive attributes.
Using supIPM as a fairness penalty, we developed supIPM-FRL to learn representations with worst-case fairness guarantees.
Our theoretical analysis established a uniform convergence rate for the empirical supIPM.
We then used the empirical supIPM to bound population $\operatorname{GDP}_{\infty}$ uniformly over heads of bounded H\"older norm.
The supIPM-FRL also enables efficient mini-batch learning.
Experiments further show lower worst-case disparity than existing methods at comparable prediction performance.
...
Figure~\ref{fig:app-c1} repeats the worst-case comparison of Figure~\ref{fig:worst-case} with the same third panel.

\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.8\textwidth]{appendix/figA2_worst_head_budgets_adult_c1.pdf}\\[1pt]
  \includegraphics[width=0.8\textwidth]{appendix/figA2_worst_head_budgets_crime_c1.pdf}\\[1pt]
  \includegraphics[width=0.8\textwidth]{appendix/figA2_worst_head_budgets_acs_c1.pdf}
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.72\textwidth]{appendix/figA7_graph_gdp_w_kernel.pdf}
  \caption{Pokec-z (left) and Pokec-n (right): prediction performance against the kernel GDP.}
  \label{fig:app-graph-gdp-kernel}
\end{figure}

\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.72\textwidth]{appendix/figA7_graph_gdp_wo_kernel.pdf}
  \caption{Pokec-z (left) and Pokec-n (right): prediction performance against the binned GDP.}
  \label{fig:app-graph-gdp-binned}
\end{figure}

\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.72\textwidth]{appendix/figA7_graph_sup_ipm.pdf}
  \caption{Pokec-z (left) and Pokec-n (right): prediction performance against the empirical supIPM.}
  \label{fig:app-graph-supipm}
\end{figure}

\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.72\textwidth]{appendix/figA7_graph_hgr.pdf}
  \caption{Pokec-z (left) and Pokec-n (right): prediction performance against the HGR maximal correlation.}
  \label{fig:app-graph-hgr}
\end{figure}

\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.72\textwidth]{appendix/figA7_graph_mi_y_s.pdf}
  \caption{Pokec-z (left) and Pokec-n (right): prediction performance against the mutual information $I(\\hat{Y};S)$.}
...
Appendix~\ref{app:mini-batch} shows that supIPM-FRL admits mini-batch learning.  The MMD penalty of FREM carries no such guarantee, so it should be trained on the batch, that is, on the whole training sample.  Table~\ref{tab:cost} reports one gradient step on Adult under these two regimes.  It also covers sup-MMD, the variant of Appendix~\ref{app:ablation-ipm-class} with the MMD in place of the ReLU discriminators.
...
The MMD evaluates the kernel at every pair of representations, so its cost grows quadratically with the number of observations.  On the whole batch, this cost exceeds the $24$ GB of the GPU.  In practice, they are therefore trained with mini-batches, without the unbiasedness such as Appendix~\ref{app:mini-batch}.  Figure~\ref{fig:cost} gives the computational cost as the mini-batch grows.  FREM already needs $8.5$ GB at $1{,}024$ observations.  The memory of supIPM-FRL grows linearly and the whole training sample takes $129$ MB, while its time per step is almost same with the mini-batch.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{appendix/figAC_cost.pdf}
...
This computational advantage, together with the guarantee of Appendix~\ref{app:mini-batch}, is the main reason for the ReLU discriminators of Section~\ref{sec:discriminator-choice}.
