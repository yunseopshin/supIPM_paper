...
\paragraph{Worst-case downstream disparity.}  The supremum over $\mathcal{F}_c$ in Equation~(\ref{eq:worst-case-gdp}) is approximated by the head
\begin{equation}
  \hat{f}_c:=\argmax_{f\in\mathcal{F}_c}\Bigl\{\widehat{\operatorname{GDP}}_{\infty}(f\circ h)-10^{-3}\,\widehat{\mathcal{R}}(f\circ h)\Bigr\},
  \label{eq:worst-case-estimate}
\end{equation}
where $\widehat{\mathcal{R}}$ is the empirical supervised risk and both terms are computed on the training representation; the reported value is $\widehat{\operatorname{GDP}}_{\infty}(\hat{f}_c\circ h)$ on the test representation.  The maximization is carried out by projected gradient ascent over $(\mathbf{w},b)$ from eight initial heads, with $\mathbf{w}$ projected onto the ball of radius $c$ after every step.
