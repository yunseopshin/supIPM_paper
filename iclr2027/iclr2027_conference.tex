...
We generate $S$ uniformly on an interval and an input $\mathbf{X}$ whose coordinates are independent of $S$, except one that is shifted when $S$ lies near a value $s_0$.  The input therefore depends on $S$ only in a neighborhood of $s_0$, and the outcome $Y$ does not depend on $S$, so a representation can discard this dependence without loss of prediction performance.
% so this dependence can be removed without loss of prediction performance.  
The full setup is given in Appendix~\ref{app:synth-experiments}.
