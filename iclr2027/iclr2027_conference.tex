...
Thus, for fixed $T$, the number of evaluations per iteration grows linearly with $r$, allowing to extend supIPM-FRL for
...
Assumption~\ref{as:encoder} requires that the encoder class $\mathcal{E}_n$ has the covering number 
...
We compare supIPM-FRL with FREM by varying $\lambda$ in (\ref{eq:frem-objective}) and (\ref{eq:supipm-penalized-objective}), respectively, and measure fairness violation by the worst-case downstream disparity over $\mathcal{F}_1$.
