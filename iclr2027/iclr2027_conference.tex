...
\paragraph{Results.}  At comparable prediction performance, supFR attains the lowest worst-case downstream disparity in the left panel, and its $\widehat{\Delta}_{\hat{f}_1}(s)$ is the smallest over the whole sensitive domain in the right panel.  $\widehat{\Delta}_{\hat{f}_1}(s)$ of $\mathrm{supFR}_{\mathrm{MMD}}$ has a profile close to that of FREM away from $s_0$, whereas at $s_0$ its peak is about half of that of FREM.  Since the two share the discriminator class $\mathcal{V}$ and differ only in taking the supremum instead of the average over $s$, this difference is due to the supremum: for every $f\in\mathcal{V}$ and $s\in\mathcal{S}$,
\begin{equation*}
  \Delta_{f}(s)
