...
Appendices~\ref{app:synth-experiments} and~\ref{app:real-experiments} give the experimental details and further results, and Appendix~\ref{app:ablation-studies} the ablation studies.
...
Appendix~\ref{app:mini-batch} shows that, for fixed inner variables, the mini-batch objective of supIPM-FRL and its gradient are unbiased for their counterparts on the whole training sample.  The MMD penalty of FREM has no such mini-batch formulation \citep{lee2026fair}, so its penalty is defined on the whole training sample.  Table~\ref{tab:cost} compares the cost of one gradient step on Adult for supIPM-FRL with a mini-batch and for FREM and sup-MMD with a single batch of $32{,}561$ observations, where sup-MMD is the variant of Appendix~\ref{app:ablation-ipm-class} with the MMD in place of the ReLU discriminators.
...
This appendix examines three choices in the penalty of supIPM-FRL on the tabular datasets: the kernel in estimator of distribution, the class of the discriminators and the number $T$ of candidate sensitive values. 
...
The discriminator class $\mathcal{V}$ determines which discrepancies between $\mathbb{P}_{\mathbf{Z}\mid S=s}$ and $\mathbb{P}_{\mathbf{Z}}$ the supremum in (\ref{eq:supipm}) detects, and hence which downstream heads the penalty controls.  We replace the ReLU class $\mathcal{V}_{\mathrm{R}}$ of (\ref{eq:relu-class}) by the sigmoid discriminators $\sigma(\theta^{\top}\mathbf{z}+\mu)$ used by sIPM-LFR \citep{kim2022learning} and by the unit ball of the Gaussian RKHS, whose IPM is the MMD used by FREM \citep{kong2025fair}. Appendix~\ref{app:real-cost} refers to the latter variant as sup-MMD.  Both variants retain the supremum over $s$, which the baselines sIPM-LFR and FREM do not. 
