...
\paragraph{Results.}  In the left panel of Figure~\ref{fig:simulation}, supIPM-FRL attains a lower worst-case downstream disparity than FREM at comparable prediction performance.
...
For each method, the circled point is the model with the smallest worst-case disparity among those that retain the performance of the model trained without a penalty.  The right panel plots $\widehat{\Delta}_{\hat{f}_1}(s)$ for these two models.  For FREM it has a peak at $s_0$, whereas for supIPM-FRL it is markedly smaller there.
This is the limitation of averaging over the sensitive attribute in Figure~\ref{fig:motivation}: only a small proportion of the sensitive values fall in the narrow interval around $s_0$, so a large discrepancy between the conditional and the marginal distributions in this interval has only a small effect on the EIPM of Equation~(\ref{eq:eipm}), and reducing the EIPM need not make these distributions close at every sensitive value.
...
\paragraph{Baselines.}
We compare supIPM-FRL with seven baselines for fair representation learning (FRL) or for learning fair prediction models.
Table~\ref{tab:baselines} summarizes the sensitive attribute and the learning target of each method in its original formulation, and Appendix~\ref{app:real-baselines} describes the fairness terms and the penalty values.
...
LAFTR, MMD and sIPM-LFR require a binary $S$, which we obtain by splitting $S$ at its median.
All methods use the encoder and the prediction head of Appendix~\ref{app:real-settings}; for each method we vary its penalty parameter over a grid and train with five random seeds.
...
\caption{Overview of compared algorithms.}
\label{tab:baselines}
\begin{tabular}{lll}
\toprule
Method & sensitive attribute & learning target \\
\midrule
supIPM-FRL & continuous & representation \\
FREM \citep{kong2025fair} & continuous & representation \\
Reg-GDP \citep{jiang2022generalized} & continuous or discrete & prediction model \\
Reg-HGR \citep{mary2019fairness} & continuous or discrete & prediction model \\
ADV \citep{zhang2018mitigating} & continuous or discrete & prediction model \\
...
At comparable prediction performance, supIPM-FRL is similar to the other methods at the prediction level, whereas its worst-case disparity is markedly smaller than that of every other method on all three datasets.
...
Figure~\ref{fig:profile-adult} plots $\widehat{\Delta}_{\hat{f}_1}(s)$ on Adult for supIPM-FRL and FREM at a test accuracy within $0.004$ of the model trained without a penalty.
...
\begin{figure}[H]
  \centering
  \includegraphics[trim=0 22.4 0 2.2, clip, width=0.72\textwidth]{appendix/figA7_graph_gdp_w_kernel.pdf}\\[1pt]
  \includegraphics[trim=0 22.4 0 2.2, clip, width=0.72\textwidth]{appendix/figA7_graph_gdp_wo_kernel.pdf}\\[1pt]
  \includegraphics[trim=0 3.3 0 2.2, clip, width=0.72\textwidth]{appendix/figA7_graph_sup_ipm.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[trim=0 23.2 0 2.2, clip, width=0.62\textwidth]{appendix/figA7_graph_hgr.pdf}\\[1pt]
  \includegraphics[trim=0 3.3 0 2.2, clip, width=0.62\textwidth]{appendix/figA7_graph_mi_y_s.pdf}
...
Appendix~\ref{app:mini-batch} shows that, for fixed inner variables, the mini-batch objective of supIPM-FRL and its gradient are unbiased for their counterparts on the whole training sample.  The MMD penalty of FREM has no such mini-batch formulation \citep{lee2026fair}, so its penalty is defined on the whole training sample.  Table~\ref{tab:cost} compares the cost of one gradient step on Adult for supIPM-FRL with a mini-batch and for FREM and sup-MMD with a single batch of $32{,}000$ observations, where sup-MMD is the variant of Appendix~\ref{app:ablation-ipm-class} with the MMD in place of the ReLU discriminators.
...
This appendix examines three choices in the penalty of supIPM-FRL on the tabular datasets: the shape of the smoothing kernel, the class of the discriminators and the number $T$ of candidate sensitive values. 
...
The shape of the kernel $K_{\gamma}$ of Equation~(\ref{eq:supipm-estimators}) determines how the sensitive values neighboring $s$ are weighted.  Writing $u$ for the distance between two sensitive values in units of the bandwidth, we compare the Gaussian shape $\exp(-u^{2}/2)$ with the Epanechnikov shape $(1-u^{2})_{+}$, the triangular shape $(1-|u|)_{+}$ and the box shape $\mathbf{1}\{|u|\le1\}$, the last three of which vanish beyond one bandwidth from $s$.  The four are used at the same bandwidth, and their weights are normalized to sum to one, so that only the shape of the weighting changes.
