...
Moreover, mini-batch learning is not possible for supIPM with MMD \citep{lee2026fair}, and thus  
$O(n^2)$ memory is required. In contrast, we show in Appendix~\ref{app:mini-batch} that supIPM with the ReLU-IPM admits mini-batch learning.
...
In contrast, supIPM-FRL controls the largest ReLU-IPM between $\mathbb{P}_{\mathbf{Z}\mid S=s}$ and $\mathbb{P}_{\mathbf{Z}}$ over the sensitive domain, and Corollary~\ref{cor:certificate} bounds $\Delta_f(s)$ on $\mathcal{S}_n$ for every head $f$ of bounded H\"older norm in terms of the empirical supIPM.
Figure~\ref{fig:profile-adult} plots $\widehat{\Delta}_{\hat{f}_1}(s)$ on Adult for supIPM-FRL and FREM at a test accuracy within $0.004$ of the model trained without a fairness penalty.
% For each method, we take the setting with the smallest worst-case disparity among these settings.
It is small over the whole sensitive domain for supIPM-FRL, whereas for FREM, it is substantially larger over most of it.
...
We write $\Pi_{A}$ for the projection onto a set $A$.  One compass search step with step size $\alpha_s$ evaluates $Q$ at the candidate $s^{(t)}$ and at the two points $\Pi_{\mathcal{S}}(s^{(t)}\pm\alpha_s)$, moves the candidate to the point with the largest value, and halves $\alpha_s$.  The $T$ candidates are moved in parallel.  The search starts from the step size $\alpha_{s,0}$ and stops once $\alpha_s<\gamma/8$.
...
\begin{algorithmic}[1]
\REQUIRE sample $\mathcal{D}$, supervised loss $\ell$, kernel $K_{\gamma}$ with bandwidth $\gamma$, penalty parameter $\lambda$, number of candidates $T$, number of discriminators $M$, number of iterations $R$, number of inner updates $J_v$, initial step size $\alpha_{s,0}$ of the compass search, step sizes $\alpha_v$ and $\alpha$
...
\FOR{$r=1,\ldots,R$}
  \FOR{$j=1,\ldots,J_v$}
    \STATE Initialize $\mathbf{s}$ at $T$ distinct points of $\mathcal{S}$ and set $\alpha_s\leftarrow\alpha_{s,0}$ \label{alg:s-init}
    \WHILE{$\alpha_s\ge\gamma/8$}
      \STATE $s^{(t)}\leftarrow\argmax_{s\in\{s^{(t)},\;\Pi_{\mathcal{S}}(s^{(t)}-\alpha_s),\;\Pi_{\mathcal{S}}(s^{(t)}+\alpha_s)\}}\,Q(s)$ for $t\in[T]$, and $\alpha_s\leftarrow\alpha_s/2$
    \ENDWHILE
...
\toprule
batch & epochs & learning rate & weight decay & $M$ & $J_v$ & $\alpha_v$ & $T$ & $\alpha_{s,0}$ \\
\midrule
...
\toprule
Dataset & batch & epochs & learning rate & weight decay & $M$ & $J_v$ & $\alpha_v$ & $T$ & $\alpha_{s,0}$ \\
\midrule
...
Appendix~\ref{app:mini-batch} shows that, for fixed inner variables, the mini-batch objective of supIPM-FRL and its gradient are unbiased for their counterparts on the whole training sample.  The MMD penalty of FREM has no such mini-batch formulation \citep{lee2026fair}, so its penalty is defined on the whole training sample.  Table~\ref{tab:cost} compares the cost of one gradient step on Adult for supIPM-FRL with a mini-batch and for FREM and supMMD-FRL with a single batch of $32{,}561$ observations, where supMMD-FRL replaces the ReLU-IPM in supIPM-FRL by the MMD.
