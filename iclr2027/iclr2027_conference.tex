...
\end{equation}
over $\lVert\mathbf{w}\rVert_2\le c$ by projected gradient descent, where $\ell$ is the supervised loss of Equation~(\ref{eq:supipm-penalized-objective}) and $\lambda_h=10^{3}$, so that the disparity term dominates.  The minimizer $\hat{f}_c:=f_{\hat{\mathbf{w}},\hat{b}}$ approximates the head attaining the supremum in Equation~(\ref{eq:worst-case-gdp}).  The reported value is the disparity of this head on the test data, $\widehat{\operatorname{GDP}}_{\infty}(h;\mathcal{F}_c):=\widehat{\operatorname{GDP}}_{\infty}(\hat{f}_c\circ h)$; the optimization is described in Appendix~\ref{app:worst-case}.
