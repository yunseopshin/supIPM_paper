...
\paragraph{Measures.}  Prediction performance is measured by the test accuracy for classification and by the test $1-\mathrm{MAE}$ for regression, as in \citet{kong2025fair}.  Fairness is measured on the test data by the prediction-level disparity $\operatorname{GDP}_{\infty}(f_{\psi}\circ h_{\phi})$ in Equation~(\ref{eq:linfty-gdp}), the disparity of the head trained jointly with the encoder.  Since a representation may later serve other prediction heads, we also measure the largest disparity that a downstream head can attain on it, following \citet{coston2021characterizing,jovanovic2023fare,langlade2025fairness,luo2026fair}.  For $c>0$, let $\mathcal{F}_c:=\{\sigma(\mathbf{w}^\top\mathbf{z}+b):\lVert\mathbf{w}\rVert_2\le c,\ b\in\mathbb{R}\}$, where $\sigma$ is the logistic function, and define the worst-case downstream disparity
\begin{equation}
  \operatorname{GDP}_{\infty}(h;\mathcal{F}_c):=\sup_{f\in\mathcal{F}_c}\operatorname{GDP}_{\infty}(f\circ h).
  \label{eq:worst-case-gdp}
\end{equation}
Every head in $\mathcal{F}_c$ has a bounded H\"older norm, so Corollary~\ref{cor:certificate} bounds Equation~(\ref{eq:worst-case-gdp}) over the interior region $\mathcal{S}_n$ in terms of the empirical supIPM.  For a head $f$ we write $\Delta(s):=|\mathbb{E}[f(\mathbf{Z})\mid S=s]-\mathbb{E}[f(\mathbf{Z})]|$, so that $\operatorname{GDP}_{\infty}(f\circ h)=\sup_{s}\Delta(s)$.

\paragraph{Computation of the worst-case disparity.}  The supremum in Equation~(\ref{eq:worst-case-gdp}) is approximated by a head of $\mathcal{F}_c$ found with the encoder held fixed.  Let $\widehat{\operatorname{GDP}}_{\infty}$ be the kernel estimate of $\operatorname{GDP}_{\infty}$ and $\widehat{\mathcal{R}}$ the empirical supervised risk, both on the training data.  For $\lambda_h>0$ we compute
\begin{equation}
...
\end{equation}
by projected gradient descent over $(\mathbf{w},b)$ and report $\operatorname{GDP}_{\infty}(\hat{f}_c\circ h)$ estimated on the test data.  With $\lambda_h=10^{3}$ the disparity dominates the objective, so that $\hat{f}_c$ approximates the supremum in Equation~(\ref{eq:worst-case-gdp}).  The estimates and the optimization are described in Appendix~\ref{app:worst-case}.
...
We generate $S$ uniformly on an interval; a single input variable depends on $S$ through a term concentrated near a value $s_0$, and the outcome $Y$ does not depend on $S$, so this dependence can be removed without loss of prediction performance.
...
\begin{figure}[!h]
  ...
  \caption{Left: prediction performance against $\operatorname{GDP}_{\infty}(h_{\phi};\mathcal{F}_1)$; the upper left is favorable.  Right: estimated $\Delta(s)$ of the heads $\hat{f}_1$ at the two circled points.}
...
At the circled points, the configurations with the smallest disparity that retain the prediction performance of the model trained without a penalty, FREM still exhibits a peak in $\Delta(s)$ at $s_0$, whereas the $\Delta(s)$ of supIPM-FRL at $s_0$ is markedly smaller and not distinguishable from its values elsewhere. 
...
\paragraph{Prediction and fairness measures.}  All measures are computed on the test data, indexed here by $i=1,\ldots,n$.  Prediction performance is the accuracy for classification and $1-\mathrm{MAE}$ for regression.  For classification, $f_{\psi}$ denotes the predicted probability, so that $f_{\psi}\circ h_{\phi}$ and the heads of $\mathcal{F}_c$ take values in $(0,1)$.  Fairness is measured by the prediction-level disparity $\operatorname{GDP}_{\infty}(f_{\psi}\circ h_{\phi})$ and by the worst-case downstream disparity $\operatorname{GDP}_{\infty}(h_{\phi};\mathcal{F}_c)$ of Equation~(\ref{eq:worst-case-gdp}), with $c=1$ except in Appendix~\ref{app:real-tabular}.  Both use the estimates
\begin{equation}
...
\paragraph{Worst-case downstream disparity.}  The head $\hat{f}_c$ of Equation~(\ref{eq:worst-case-estimate}) is computed with $\widehat{\operatorname{GDP}}_{\infty}$ of Equation~(\ref{eq:gdp-estimate}) and the empirical supervised risk $\widehat{\mathcal{R}}$ formed from the training sample, by projected gradient descent over $(\mathbf{w},b)$ from eight initial heads, with $\mathbf{w}$ projected onto the ball of radius $c$ after every step.  The reported value is $\widehat{\operatorname{GDP}}_{\infty}(\hat{f}_c\circ h)$ on the test sample.
...
\begin{figure}[!htb]
  ...
  \caption{Prediction performance against the prediction-level disparity $\operatorname{GDP}_{\infty}(f_{\psi}\circ h_{\phi})$ on Crime (top) and ACSIncome (bottom).}
...
\begin{figure}[!htb]
  ...
  \caption{Prediction performance against the worst-case downstream disparity $\operatorname{GDP}_{\infty}(h_{\phi};\mathcal{F}_{4})$ on Adult (top), Crime (middle) and ACSIncome (bottom).}
  \label{fig:app-c4}
\end{figure}
...
\begin{figure}[!htb]
  ...
  \caption{Prediction performance against the worst-case downstream disparity $\operatorname{GDP}_{\infty}(h_{\phi};\mathcal{F}_{8})$ on Adult (top), Crime (middle) and ACSIncome (bottom).}
  \label{fig:app-c8}
\end{figure}
\FloatBarrier
...
\begin{figure}[!htb]
  ...
  \caption{Pokec-z (left) and Pokec-n (right): prediction performance against the prediction-level disparity $\operatorname{GDP}_{\infty}(f_{\psi}\circ h_{\phi})$ (top) and the worst-case downstream disparity $\operatorname{GDP}_{\infty}(h_{\phi};\mathcal{F}_1)$ (bottom).}
  \label{fig:app-graph}
\end{figure}
\FloatBarrier
...
\begin{table}[!htb]
\centering
\caption{Prediction-level disparity $\operatorname{GDP}_{\infty}(f_{\psi}\circ h_{\phi})$ and worst-case downstream disparity $\operatorname{GDP}_{\infty}(h_{\phi};\mathcal{F}_1)$ at a matched level of prediction performance on Adult, Crime and ACSIncome.}
\label{tab:matched}
\footnotesize
