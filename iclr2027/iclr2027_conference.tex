...
\paragraph{Prediction and fairness measures.}  All measures are computed on the test data, indexed here by $i=1,\ldots,n$.  Prediction performance is the accuracy for classification and $1-\mathrm{MAE}$ for regression.  Fairness is measured by the prediction-level $L_{\infty}$-GDP of the trained model $f_{\psi}\circ h_{\phi}$ and by the worst-case downstream $L_{\infty}$-GDP over $\mathcal{F}_c$ of Equation~(\ref{eq:worst-case-gdp}), with $c=1$ except in Appendix~\ref{app:real-tabular}.  Both use the estimates
\begin{equation}
...
\end{equation}
where $\widehat{w}_{\tilde{\gamma}}(i;s)$ is the kernel weight of Equation~(\ref{eq:supipm-estimators}) with a Gaussian kernel of bandwidth $\tilde{\gamma}=(3n/4)^{-1/5}$ \citep{silverman2018density}, so that the first sum is the Nadaraya--Watson estimate of $\mathbb{E}[f(\mathbf{Z})\mid S=s]$, and $\mathcal{Q}$ is a grid of spacing $10^{-3}$ between the $5\%$ and the $95\%$ quantiles of $S$, the empirical counterpart of the interior region $\mathcal{S}_n$ of Section~\ref{sec:theory}.
...
\end{equation}
where the superscript $\mathrm{tr}$ indicates that Equation~(\ref{eq:gdp-estimate}) and the empirical supervised risk $\widehat{\mathcal{R}}$ are formed from the training sample.  The maximization is carried out by projected gradient ascent over $(\mathbf{w},b)$ from eight initial heads, with $\mathbf{w}$ projected onto the ball of radius $c$ after every step.
