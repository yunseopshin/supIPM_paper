...
\paragraph{Estimating the worst-case disparity.}  We search for the head attaining the supremum in Equation~(\ref{eq:worst-case-gdp}), with the encoder held fixed and with $\widehat{\operatorname{GDP}}_{\infty}$ and the empirical supervised risk $\widehat{\mathcal{R}}$ computed on the training data.  For $\lambda_h>0$ we find
\begin{equation}
