...
\begin{figure}[t]
  ...
  \caption{The average discrepancy can be small even when the discrepancy is large in a low-probability region near $s_0$.}
...
For each method, the circled point in the left panel marks the model with the smallest worst-case disparity among those that retain the prediction performance of the model trained without a fairness penalty, and the right panel plots $\widehat{\Delta}_{\hat{f}_1}(s)$ for these two models.  For FREM it has a peak at $s_0$, whereas for supIPM-FRL it is markedly smaller there.
...
\paragraph{Datasets.}  We use three tabular datasets: Adult \citep{adult_2}, where $Y$ indicates whether income exceeds \$50k and $S$ is age; Crime \citep{communities_and_crime_183}, where $Y$ is the violent crime rate and $S$ is the proportion of Black residents; and ACSIncome \citep{ding2021retiring}, from the 2018 American Community Survey, where $Y$ is a binary income class and $S$ is age.  Table~\ref{tab:datasets} in Appendix~\ref{app:real-datasets} gives further details, and Appendix~\ref{app:real-additional} reports results on the graph datasets Pokec-z and Pokec-n \citep{takac2012data}.

\paragraph{Baselines.}
We compare supIPM-FRL with seven baselines for fair representation learning (FRL) or for learning fair prediction models.
Table~\ref{tab:baselines} summarizes, for each method, the sensitive attribute and the learning target in its original formulation and in this study, and Appendix~\ref{app:real-baselines} describes the fairness terms and the penalty values.
...
\caption{Methods compared in Section~\ref{sec:exp-real}.}
\label{tab:baselines}
\begin{tabular}{lllll}
\toprule
 & \multicolumn{2}{c}{original formulation} & \multicolumn{2}{c}{this study} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
Method & sensitive attribute & learning target & sensitive attribute & learning target \\
\midrule
supIPM-FRL & continuous & representation & continuous & representation \\
FREM \citep{kong2025fair} & continuous & representation & continuous & representation \\
Reg-GDP \citep{jiang2022generalized} & continuous or discrete & prediction model & continuous & prediction model \\
Reg-HGR \citep{mary2019fairness} & continuous or discrete & prediction model & continuous & prediction model \\
ADV \citep{zhang2018mitigating} & continuous or discrete & prediction model & continuous & representation \\
LAFTR \citep{madras2018learning} & binary & representation & binary, median split & representation \\
MMD \citep{deka2023mmd} & binary & representation & binary, median split & representation \\
sIPM-LFR \citep{kim2022learning} & binary & representation & binary, median split & representation \\
\bottomrule
\end{tabular}
\vspace{-6pt}
\end{table}
...
Figure~\ref{fig:adult} compares the methods by the prediction-level disparity and Figure~\ref{fig:worst-case} by the worst-case downstream disparity over $\mathcal{F}_1$ on the three datasets.
...
At comparable prediction performance, the prediction-level disparity of supIPM-FRL is similar to that of the other methods, whereas its worst-case downstream disparity is markedly smaller than that of every other method on all three datasets.
Table~\ref{tab:matched-adult} reports the two disparities on Adult at test accuracy $0.838$--$0.846$, each entry being the smallest value among the models of that method in this range as in Appendix~\ref{app:real-matched}, and the worst-case disparity of supIPM-FRL is lower than that of every baseline.
...
The baselines do not directly control the dependence between the representation and $S$ at every sensitive value, so they may attain a small disparity for the trained head while the representation still depends on $S$, and a downstream head can exploit that dependence, which raises the worst-case disparity.

In contrast, supIPM-FRL controls the largest ReLU-IPM between $\mathbb{P}_{\mathbf{Z}\mid S=s}$ and $\mathbb{P}_{\mathbf{Z}}$ over the sensitive domain, and Corollary~\ref{cor:certificate} then bounds $\Delta_f(s)$ on the interior region $\mathcal{S}_n$, for every prediction head $f$ of bounded H\"older norm, by a function of the empirical supIPM.
Figure~\ref{fig:profile-adult} plots $\widehat{\Delta}_{\hat{f}_1}(s)$ on Adult for supIPM-FRL and FREM at a test accuracy within $0.004$ of that of the model trained without a penalty.
% For each method, we take the setting with the smallest worst-case disparity among these settings.
At this comparable accuracy, $\widehat{\Delta}_{\hat{f}_1}(s)$ of supIPM-FRL is small over the whole sensitive domain, whereas that of FREM is substantially larger over most of it.
...
The sensitive attribute enters the input only through $X_{\mathrm{loc}}$ and does not enter the outcome equation, so $Y$ is conditionally independent of $S$ given $\mathbf{X}_{\mathrm{task}}$.  A representation may therefore discard $X_{\mathrm{loc}}$, and with it the dependence on $S$ near $s_0$, without losing the information needed to predict $Y$.  Since $Y\in(0,1)$, the task is regression with the squared-error loss, and prediction performance is measured by the test $1-\mathrm{MAE}$.
...
Table~\ref{tab:baselines} in Section~\ref{sec:exp-real} classifies the baselines by the sensitive attribute and the learning target of their original formulation and of this study.  The baselines share the encoder, the prediction head and the training settings of Appendix~\ref{app:real-settings}, and differ from supIPM-FRL in their fairness term and in the range of its penalty parameter, as follows.
\begin{itemize}
...
This subsection collects the results omitted from Section~\ref{sec:exp-real}.  In every figure on the tabular datasets, the left panels show the methods that use the continuous $S$, and the middle and the right panels show the methods that require a binary $S$, with $S$ split at its median as in Section~\ref{sec:exp-real} and at its mean, respectively.
...
Figures~\ref{fig:app-graph-gdp-kernel} and~\ref{fig:app-graph-mi} compare the methods on the two graphs by the remaining measures of Appendix~\ref{app:worst-case}.  Under the kernel and the binned estimates of GDP, supIPM-FRL attains the smallest values on both graphs.  Under the empirical supIPM, its curve overlaps with that of FREM, and both lie well below Reg-GDP and Reg-HGR.  Under the HGR maximal correlation and the mutual information, Reg-HGR attains the smallest values and supIPM-FRL is comparable to FREM.

\begin{figure}[!htb]
  \centering
  \includegraphics[trim=0 22.4 0 2.2, clip, width=0.78\textwidth]{appendix/figA7_graph_gdp_w_kernel.pdf}\\[1pt]
  \includegraphics[trim=0 22.4 0 2.2, clip, width=0.78\textwidth]{appendix/figA7_graph_gdp_wo_kernel.pdf}\\[1pt]
  \includegraphics[trim=0 3.3 0 2.2, clip, width=0.78\textwidth]{appendix/figA7_graph_sup_ipm.pdf}
...
\begin{figure}[!htb]
  \centering
  \includegraphics[trim=0 23.2 0 2.2, clip, width=0.78\textwidth]{appendix/figA7_graph_hgr.pdf}\\[1pt]
  \includegraphics[trim=0 3.3 0 2.2, clip, width=0.78\textwidth]{appendix/figA7_graph_mi_y_s.pdf}
...
\begin{figure}[!htb]
  ...
  \caption{Prediction performance against the prediction-level disparity (top) and the worst-case downstream disparity over $\mathcal{F}_1$ (bottom) for supIPM-FRL trained with each smoothing kernel, on Adult (left), Crime (middle) and ACSIncome (right).}
...
\begin{figure}[!htb]
  ...
  \caption{Prediction performance against the prediction-level disparity (top) and the worst-case downstream disparity over $\mathcal{F}_1$ (bottom) for supIPM-FRL trained with each discriminator class, on Adult (left), Crime (middle) and ACSIncome (right).}
...
\begin{figure}[!htb]
  ...
  \caption{Prediction performance against the prediction-level disparity (top) and the worst-case downstream disparity over $\mathcal{F}_1$ (bottom) for supIPM-FRL trained at $T\in\{5,10,15,17,20,25,30\}$, on Adult (left), Crime (middle) and ACSIncome (right).}
