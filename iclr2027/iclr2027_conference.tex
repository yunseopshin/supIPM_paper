...
\end{equation*}
with $s_0=0.5$, $\tau=0.043$, standard normal $\varepsilon_1,\varepsilon_2$, a fixed $\mathbf{u}$ and the logistic function $\sigma$.  Thus $\mathbf{X}$ depends on $S$ only through $X_{\mathrm{loc}}$, which is shifted only for $S$ near $s_0$, and $Y$ does not depend on $S$.
