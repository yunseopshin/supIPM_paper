...
The discriminator class $\mathcal{V}$ determines which discrepancies between $\mathbb{P}_{\mathbf{Z}\mid S=s}$ and $\mathbb{P}_{\mathbf{Z}}$ the supremum in (\ref{eq:supipm}) detects, and hence which downstream heads the penalty controls.  To examine its effect on $\mathrm{FR}_{\sup}$, we replace the ReLU class $\mathcal{V}_{\mathrm{R}}$ of (\ref{eq:relu-class}) either by the sigmoid discriminators $\sigma(\theta^{\top}\mathbf{z}+\mu)$ of sIPM-LFR \citep{kim2022learning} or by the unit ball of the Gaussian RKHS, whose IPM is the MMD used by FREM \citep{kong2025fair}.  Both variants retain the supremum over $s$ in (\ref{eq:supipm}) and differ from $\mathrm{FR}_{\sup}$ only in the discriminator class, whereas sIPM-LFR and FREM do not take this supremum.  We write $\mathrm{FR}_{\sup\mathrm{MMD}}$ for the variant with the MMD; Appendix~\ref{app:real-cost} denotes the same variant by supMMD-FRL.  The architecture, the training settings of Appendix~\ref{app:real-settings} and the measures of Appendix~\ref{app:worst-case} are unchanged, and the penalty values are chosen separately for each discriminator class.
...
\begin{figure}[H]
  ...
  \caption{Left: test accuracy against $\widehat{\operatorname{GDP}}_{\infty}(h_{\phi};\mathcal{F}_1)$ on Adult for $\mathrm{FR}_{\sup}$, FREM and $\mathrm{FR}_{\sup\mathrm{MMD}}$ (sup-MMD in the legend).  Right: $\widehat{\Delta}_{\hat{f}_1}(s)$ for the three models circled in the left panel, chosen at matched test accuracy.}
