...
A critical limitation, however, in controlling the average fairness violation is that  under-represented sensitive groups could be highly discriminated against.
...
Theoretically, the class of discriminators for ReLU-IPM is much larger than that of MMD. Thus, controlling supIPM with $\operatorname{IPM}_{\mathcal{V}_{\mathrm{R}}}$
...
We generate an input $\mathbf{X}:=(\mathbf{X}_{\mathrm{task}},\mathbf{X}_{\mathrm{noise}},S)$ and an outcome $Y$ as follows.
We independently draw $S$ on $[0,1]$ with density
$p(s) \propto 1-0.9\exp\{-(s-s_0)^2/(2\tau^2)\}$, $\mathbf{X}_{\mathrm{task}}\sim N(\mathbf{0},\mathbf{I}_4)$, $\mathbf{X}_{\mathrm{noise}}\sim N(\mathbf{0},\mathbf{I}_3)$ and $\varepsilon_1,\varepsilon_2\sim N(0,1)$, and set
\begin{equation*}
X_{\mathrm{loc}}:=3\exp\!\left\{-\frac{(S-s_0)^2}{2\tau^2}\right\}+0.5\,\varepsilon_1,
\qquad
Y:=\sigma\!\left(\boldsymbol{\upsilon}^\top\mathbf{X}_{\mathrm{task}}+X_{\mathrm{loc}}+0.3\,\varepsilon_2\right),
...
To confirm this conjecture, we compare supFR with FREM by varying $\lambda$ in (\ref{eq:frem-objective}) and (\ref{eq:supipm-penalized-objective}), respectively, and measure the worst-case downstream disparity over $\mathcal{F}_1$.
...
The baselines do not directly control disparity at every sensitive value, so the worst-case fairness violation may be large with other downstream heads.
In contrast, supFR controls the largest ReLU-IPM between $\mathbb{P}_{\mathbf{Z}\mid S=s}$ and $\mathbb{P}_{\mathbf{Z}}$ over the sensitive domain, and Corollary~\ref{cor:certificate} implies that the empirical supIPM yields a bound on $\Delta_f(s)$ over $\mathcal{S}_n$ for every head $f$ of bounded H\"older norm.
...
\begin{remark}
    \label{rem:sup-mmd}
    Unlike the worst-case disparity shown in Figure \ref{fig:simulation}, FREM exhibits high $\widehat{\Delta}_{\hat{f}_1}(s)$ across the sensitive domain in Figure~\ref{fig:profile-adult}.
A possible explanation is that the discriminator class of MMD used in FREM is smaller than that of ReLU-IPM, as discussed in Section~\ref{sec:discriminator-choice}.
We further examine the effect of the choice of a discriminator class in Appendix~\ref{app:ablation-ipm-class}.
\end{remark}
...
We compare the methods under five further fairness measures and on the graph datasets in Appendix~\ref{app:real-additional}, and the computational cost of supFR and FREM in Appendix~\ref{app:real-cost}. Appendix~\ref{app:ablation-ipm-class} examines the effect of the discriminator class $\mathcal{V}$, and Appendix~\ref{app:ablation-studies} the effects of the kernel $K$ in (\ref{eq:supipm-estimators}) and of the number $T$ of initial values of $s$.
...
Future work includes extending supIPM to fairness criteria conditional on the outcome, such as equalized odds or equal opportunity.
...
In this work, we used generative AI tools to assist in the writing of proofs: the tools suggested intermediate steps and explicit constants for the proofs in Appendix~\ref{app:proofs}, which we checked line by line and rewrote where necessary.
The definitions, the assumptions, the statements of Theorem~\ref{thm:uniform} and Corollary~\ref{cor:certificate}, and the overall proof strategy are our own.
We also used generative AI tools to edit the manuscript for readability and to draft parts of the text from our notes, and to identify and retrieve relevant literature; every cited work was read and checked against the original source by the authors.
% The following paragraph lists AI-assisted tasks beyond the three items checked in the submission form; delete or edit it if it does not reflect the authors' use.
In addition, we used generative AI tools to write and edit parts of the software that implements supFR and the experimental pipeline, to create the scientific figures from our experimental results, to obtain feedback on the design of the experiments, to formulate and verify auxiliary statements in the appendices, and to organize and interpret the experimental results.
All code was reviewed and tested by the authors, every figure was checked against the underlying results, and every interpretation and auxiliary statement was verified by us.
We have not used generative AI tools to generate data sets, to propose the research question or the hypotheses, to translate, or to clean or reformat data.
We have reviewed all AI-assisted work and take responsibility for the final content of this work, including text, claims, and artifacts produced with the aid of generative AI.

\section*{Reproducibility Statement}

The assumptions and the complete proofs of Theorem~\ref{thm:uniform} and Corollary~\ref{cor:certificate} are given in Appendix~\ref{app:proofs}.
Appendix~\ref{app:algorithm} describes the optimization procedure of supFR, its mini-batch formulation, and the full training procedures in Algorithms~\ref{alg:supipm-frl} and~\ref{alg:supipm-frl-mb}.
Appendix~\ref{app:synth-experiments} specifies the data-generating process and the training settings of the simulation study.
Appendix~\ref{app:real-experiments} gives the datasets and their preprocessing, the architectures and training settings, the baselines and the ranges of their penalty parameters, and the definitions of all evaluation measures, including the estimation of the worst-case downstream disparity.
Every reported point is the mean over five random seeds, with one standard deviation shown as error bars.
The computational cost and the sensitivity of supFR to the discriminator class, the kernel, and the number of candidate sensitive values are reported in Appendices~\ref{app:real-cost}, \ref{app:ablation-ipm-class}, and~\ref{app:ablation-studies}.
...
\begin{aligned}
  Y &:= \sigma\!\left(\boldsymbol{\upsilon}^\top\mathbf{X}_{\mathrm{task}}+X_{\mathrm{loc}} + 0.3\,\varepsilon_2\right), \\
  \mathbf{X} &:= (\mathbf{X}_{\mathrm{task}},\, \, \mathbf{X}_{\mathrm{noise}}, S) \in \mathbb{R}^{8}.
\end{aligned}
\label{eq:synth-linear}
\end{equation}


The sensitive attribute enters the output only through $X_{\mathrm{loc}}$.
...
\begin{itemize}
  \item FREM \citep{kong2025fair} penalizes the empirical EIPM of (\ref{eq:eipm-estimator}) with the RKHS unit ball of a Gaussian kernel of bandwidth $1.0$.  The kernel bandwidth on $S$ is $0.03$, $0.05$ or $0.07$, and the penalty values lie between $0.01$ and $3$.
...
Figure~\ref{fig:app-prediction-level} compares the methods by the prediction-level disparity on the three tabular datasets, with the binary-$S$ methods under both splits of $S$.  On Crime and ACSIncome, supFR, FREM and Reg-GDP have nearly the same curve, and the binary-$S$ methods are close to them.

\begin{figure}[H]
  \centering
  \includegraphics[trim=0 22.1 0 3.8, clip, width=0.88\textwidth]{appendix/fig2_own_head_adult.pdf}\\[1pt]
  \includegraphics[trim=0 29.8 0 1.7, clip, width=0.88\textwidth]{appendix/fig2_own_head_crime.pdf}\\[1pt]
...
\begin{figure}[H]
  \centering
  \includegraphics[trim=0 31.4 0 2.4, clip, width=0.88\textwidth]{appendix/figA_worst_head_mlp2_adult.pdf}\\[1pt]
  \includegraphics[trim=0 31.4 0 2.8, clip, width=0.88\textwidth]{appendix/figA_worst_head_mlp2_crime.pdf}\\[1pt]
  \includegraphics[trim=0 8.3 0 2.8, clip, width=0.88\textwidth]{appendix/figA_worst_head_mlp2_acs.pdf}
...
Figure~\ref{fig:app-ablation-ipm-class} compares the three classes on the tabular datasets; the columns are Adult, Crime and ACSIncome from left to right, the top row reports the prediction-level disparity and the bottom row the worst-case downstream disparity over $\mathcal{F}_1$.  Figure~\ref{fig:app-ablation-ipm-class-Delta} compares supFR, FREM and $\mathrm{supFR}_{\mathrm{MMD}}$ on Adult.  Its left panel shows the test accuracy against the worst-case downstream disparity over $\mathcal{F}_1$, and the circles mark the model of each method chosen at matched test accuracy as in Figure~\ref{fig:profile-adult}.  Its right panel shows $\widehat{\Delta}_{\hat{f}_1}(s)$ of the three circled models over the sensitive domain.
...
In the right panel of Figure~\ref{fig:app-ablation-ipm-class-Delta}, $\widehat{\Delta}_{\hat{f}_1}(s)$ of supFR is small over the whole sensitive domain, whereas that of $\mathrm{supFR}_{\mathrm{MMD}}$ is substantial near the left boundary and around $s\approx0.5$, with a profile close to that of FREM.  Since $\mathrm{supFR}_{\mathrm{MMD}}$ shares the discriminator class with FREM and the supremum over $s$ with supFR, this comparison suggests that the advantage of supFR over FREM in Figure~\ref{fig:profile-adult} is attributable largely to the ReLU discriminator class rather than to the replacement of the average by the supremum.  This observation is consistent with Section~\ref{sec:discriminator-choice}: the ReLU class controls the fairness violation of H\"older smooth prediction heads through (\ref{eq:relu-holder}), whereas the Gaussian RKHS unit ball contains only infinitely differentiable functions (Section~\ref{sec:relu-ipm}).  The comparison leaves the case for supIPM intact.  Computing the empirical EIPM with $\mathcal{V}_{\mathrm{R}}$ requires adversarial learning at each $S_i$ (Section~\ref{sec:relu-ipm}), whereas the empirical supIPM in (\ref{eq:empirical-supipm}) requires a single maximization over $(s,\theta,\mu)$ (Section~\ref{sec:supipm-objective}), so the supremum over $s$ renders the ReLU discriminator class computationally tractable for a continuous sensitive attribute.
...
\paragraph{Results.}  In Figure~\ref{fig:app-ablation-T}, the seven values of $T$ yield similar curves: on Crime the curves overlap, and on Adult and ACSIncome the differences between them lie within one standard deviation over the seeds and show no trend in $T$.  Within the evaluated range of $T$, the results are therefore insensitive to the number of starting points of the search.
