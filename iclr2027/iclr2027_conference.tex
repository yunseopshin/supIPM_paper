...
\paragraph{Results.}  In every figure, a point is the mean over the five seeds with bars of $\pm1$ standard deviation, a curve joins the configurations of one method with the smallest disparity at each level of prediction performance, and the upper left is favorable.  Figure~\ref{fig:adult} shows the prediction-level disparity on Adult, and Figure~\ref{fig:worst-case} the worst-case downstream disparity over $\mathcal{F}_1$ on the three datasets.  Appendix~\ref{app:real-tabular} gives the prediction-level curves on Crime and ACSIncome, which show the same pattern, and the worst-case curves over $\mathcal{F}_4$ and $\mathcal{F}_8$.  At the prediction level, the methods differ little: supIPM-FRL is close to FREM and Reg-GDP over the accuracies it attains, and the methods that discretize $S$ lie below or close to its curve.  Under the worst-case downstream disparity, the methods separate.  supIPM-FRL attains a markedly smaller disparity than every other method at the same prediction performance, on all three datasets and against both kinds of baselines; the only exception is the lowest prediction performance on Crime, where the methods that use $S$ directly coincide with it.
