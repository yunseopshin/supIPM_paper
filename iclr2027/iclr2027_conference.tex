...
\paragraph{Measures.}  Prediction performance is measured by the test accuracy for classification and the test $1-\mathrm{MAE}$ for regression.  Fairness violation of a prediction head $f$ is measured through the disparity at each sensitive value:
\begin{equation}
...
The prediction-level disparity is $\operatorname{GDP}_{\infty}(f_{\psi}\circ h_{\phi}) = \sup_{s} \Delta_{f_{\psi}}(s)$ for the head trained jointly with the encoder.  Since a representation may later serve for other heads \citep{coston2021characterizing,jovanovic2023fare,luo2026fair}, we also measure the largest disparity that any downstream head can attain.  For $c>0$, let $\mathcal{F}_c:=\{\sigma(\mathbf{w}^\top\mathbf{z}+b):\lVert\mathbf{w}\rVert_2\le c,\ b\in\mathbb{R}\}$ be the logistic heads with weight norm at most $c$, where $\sigma(t):=(1+e^{-t})^{-1}.$ We define the worst-case disparity as
\begin{equation}
...
We denote $\widehat{\Delta}_{f}$ and $\widehat{\operatorname{GDP}}_{\infty}$ the estimates of these quantities which
are defined in Appendix~\ref{app:worst-case}.
...
We compare supIPM-FRL with FREM over a grid of penalty parameters, using the worst-case downstream disparity over $\mathcal{F}_1$ as the measure of fairness violation. 
...
Table~\ref{tab:matched-adult} compares prediction-level and worst-case disparities on Adult at test accuracy $0.838$--$0.846$, following the procedure described in Appendix~\ref{app:real-additional}.
supIPM-FRL achieves lower worst-case disparity than every baseline.
...
In contrast, supIPM-FRL controls the largest ReLU-IPM between $\mathbb{P}_{\mathbf{Z}\mid S=s}$ and $\mathbb{P}_{\mathbf{Z}}$ over the sensitive domain.
Therefore, The disparity $\Delta_f(s)$ is controlled over all sensitive domain $\mathcal{S}$ for every H\"older smooth prediction head $f$. 
Figure~\ref{fig:profile-adult} shows $\widehat{\Delta}_{\hat{f}_1}(s)$ of supIPM-FRL and FREM on Adult. We consider settings with test accuracy within $0.004$ of the baseline accuracy at $\lambda=0$. 
% For each method, we take the setting with the smallest worst-case disparity among these settings.
At comparable test accuracy, $\widehat{\Delta}_{\hat{f}_1}(s)$ of supIPM-FRL is small across the sensitive domain, whereas that of FREM is substantially larger.
...
Appendix~\ref{app:real-additional} compares the methods under five further fairness measures and on the graph datasets.  The computational cost is reported in Appendix~\ref{app:real-cost}.  Appendix~\ref{app:ablation-studies} ablates three choices in the penalty, (i) the shape of the smoothing kernel, (ii) the class of the discriminators and (iii) the number of candidate sensitive values.
...
\paragraph{Prediction and fairness measures.}  All measures are computed on the test data, indexed here by $i=1,\ldots,n$.  Prediction performance is the accuracy for classification and $1-\mathrm{MAE}$ for regression.  Fairness is measured by the prediction-level disparity $\operatorname{GDP}_{\infty}(f_{\psi}\circ h_{\phi})$ and by the worst-case downstream disparity $\operatorname{GDP}_{\infty}(h_{\phi};\mathcal{F}_c)$ of Equation~(\ref{eq:worst-case-gdp}), with $c=1$ except in Figures~\ref{fig:app-c4} and~\ref{fig:app-c8}.  Both use the estimates
\begin{equation}
...
This subsection collects the results omitted from Section~\ref{sec:exp-real}.  On the tabular datasets, the methods that require a binary sensitive attribute appear in two panels of every figure, with $S$ discretized at its median as in Section~\ref{sec:exp-real} and at its mean.

\paragraph{Prediction-level disparity.}
\label{app:real-tabular}
Figure~\ref{fig:app-prediction-level} repeats the comparison of Figure~\ref{fig:adult} on the three tabular datasets.  On Crime and ACSIncome, supIPM-FRL, FREM and Reg-GDP have nearly the same curve, and the binary-$S$ methods are close to them. 
...
Figure~\ref{fig:app-c1} repeats the comparison of Figure~\ref{fig:worst-case} on the same datasets.  At a matched level of prediction performance, supIPM-FRL attains the lowest worst-case disparity on the three datasets.  The supremum over $s$ and over the discriminators constrains the representation at every sensitive value, whereas an average over $S$, or a penalty on the fitted head, leaves a dependence on $S$ that another head can exploit.
...
\paragraph{Larger norm bounds.}  Figures~\ref{fig:app-c4} and~\ref{fig:app-c8} repeat the same comparison with $c=4$ and $c=8$.  At a matched level of prediction performance, supIPM-FRL attains the lowest worst-case disparity on the three tabular datasets, and its margin over the baselines is larger than at $c=1$, so its advantage persists across the evaluated norm bounds.
...
\paragraph{Two-layer mlp heads.}  We denote
\begin{equation*}
...
\end{equation*}
for the two-layer mlp heads of the same Lipschitz bound, we repeat the comparison over $\mathcal{F}'_1$ in Figure~\ref{fig:app-mlp-head}; the heads are fitted as in Appendix~\ref{app:worst-case} with $16$ hidden units, projecting $\mathbf{W}_1$ onto the spectral-norm ball and $\mathbf{w}_2$ onto the ball of radius $c$ after every step.  At a matched level of prediction performance, the values over $\mathcal{F}'_1$ are close to those over $\mathcal{F}_1$.
...
Table~\ref{tab:matched} fixes one level of prediction performance per tabular dataset and compares the methods there.  An entry is the smallest disparity among the configurations of that method whose prediction performance lies within $0.004$ of the level, a dash marks a method with no such configuration, and the smallest value in each row is in bold.  supIPM-FRL attains the smallest value in every row except the prediction-level disparity on Crime.  Under the worst-case disparity its value is at least three times smaller than that of every baseline on the three datasets.
...
Figures~\ref{fig:app-gdp}--\ref{fig:app-supipm} replace the disparity axis by the kernel and the binned estimates of GDP, both averages over $S$, and by the empirical supIPM of the representation on the test split, all defined in Appendix~\ref{app:worst-case}.  Under the two averages, supIPM-FRL and FREM attain similar values, and the remaining methods are two to four times larger.  Under the empirical supIPM, supIPM-FRL is markedly lower than every other method on Adult and Crime, consistent with Theorem~\ref{thm:uniform}.
...
Figures~\ref{fig:app-hgr} and~\ref{fig:app-mi-y} replace the axis by the HGR maximal correlation and the mutual information between the fitted prediction and $S$.  No method attains the smallest value on the three datasets, and supIPM-FRL is comparable to FREM under both measures.  These two measures are functionals of the fitted head alone, so a method that penalizes that head can attain a small value while the representation still depends on $S$.
...
On Pokec-z and Pokec-n we compare supIPM-FRL with FREM, Reg-GDP, Reg-HGR, ADV and MMD; LAFTR and sIPM-LFR are evaluated only on the tabular datasets.  In Figure~\ref{fig:app-graph}, supIPM-FRL, FREM and Reg-GDP have similar prediction-level curves.  Under the worst-case disparity, supIPM-FRL and FREM coincide, MMD is close to them, and Reg-GDP and Reg-HGR attain disparities several times larger.  On these two graphs the supremum and the average penalty control the evaluated downstream heads equally well, whereas a penalty on the fitted head does not.
...
Figures~\ref{fig:app-graph-gdp-kernel} and~\ref{fig:app-graph-mi} replace the disparity axis by the remaining measures of Appendix~\ref{app:worst-case}.  Under the kernel and the binned estimates of GDP, supIPM-FRL attains the smallest values on both graphs.  Under the empirical supIPM it coincides with FREM, and both lie well below Reg-GDP and Reg-HGR.  Under the HGR maximal correlation and the mutual information, Reg-HGR attains the smallest values and supIPM-FRL is comparable to FREM.
...
Each ablation varies one choice, holds every other setting of Appendix~\ref{app:real-settings} fixed, and leaves the measures of Appendix~\ref{app:worst-case} unchanged.  In each figure the columns are the datasets, the top row reports the prediction-level disparity and the bottom row the worst-case downstream disparity over $\mathcal{F}_1$.
...
\paragraph{Results.}  In Figure~\ref{fig:app-ablation-kernel}, over the range of prediction performance that the four kernels share, their curves are close on the three datasets and no kernel is the lowest everywhere.  The kernels differ instead in the penalty values they admit: with a compactly supported kernel the prediction performance falls to that of a constant predictor at a smaller $\lambda$, so those curves end earlier, whereas the Gaussian kernel continues to a lower disparity.
...
The discriminator class $\mathcal{V}$ determines which discrepancies between $\mathbb{P}_{\mathbf{Z}\mid S=s}$ and $\mathbb{P}_{\mathbf{Z}}$ the supremum in Equation~(\ref{eq:supipm}) detects, and hence which downstream heads the penalty controls.  We replace the ReLU class $\mathcal{V}_{\mathrm{R}}$ of Equation~(\ref{eq:relu-class}), within the same supremum over $s$, by the sigmoid discriminators $\sigma(\theta^{\top}\mathbf{z}+\mu)$ of sIPM-LFR \citep{kim2022learning} and by the unit ball of the Gaussian RKHS, whose IPM is the MMD used by FREM \citep{kong2025fair}.  Both variants retain the supremum over $s$, which sIPM-LFR and FREM do not.  The grid of penalty values is chosen separately for each class, since the three IPMs are not on a common scale, so the classes are compared at a matched level of prediction performance.

\paragraph{Results.}  In Figure~\ref{fig:app-ablation-ipm-class}, the three discriminator classes yield similar prediction-level curves.  The ReLU and the sigmoid discriminators attain comparable worst-case disparity, with an ordering that varies with the dataset and the penalty value.  Both attain a markedly lower worst-case disparity than the MMD at comparable prediction performance.
...
The compass search over $s$ of Appendix~\ref{app:ascent} starts from $T$ candidate sensitive values.  This ablation trains supIPM-FRL at $T\in\{5,10,15,20,25,30\}$ alongside the reported $T=17$, and reads the seven settings on the same seeds and the same penalty values.

\paragraph{Results.}  In Figure~\ref{fig:app-ablation-T} the seven settings are indistinguishable: on Crime the curves coincide, and on Adult and ACSIncome the differences between them lie within one standard deviation over the seeds and show no trend in $T$.  Within this range of $T$, the search is therefore insensitive to its number of starting points.
