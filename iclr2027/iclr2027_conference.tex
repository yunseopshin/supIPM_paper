...
Table~\ref{tab:matched-adult} shows the methods on Adult at a common test accuracy, chosen as described in Appendix~\ref{app:real-additional}, where the worst-case disparity of supIPM-FRL is smaller than that of every baseline.
...
\caption{The two disparities on Adult at test accuracy $0.842$.}
...
Figure~\ref{fig:app-prediction-level} repeats the comparison of Figure~\ref{fig:adult} on the three tabular datasets.  On Crime and ACSIncome, supIPM-FRL, FREM and Reg-GDP have nearly the same curve, and the binary-$S$ methods are close to them.  As on Adult, the prediction-level disparity separates the methods little.
...
Figure~\ref{fig:app-c1} repeats the comparison of Figure~\ref{fig:worst-case} on the same datasets.  At a matched level of prediction performance, supIPM-FRL attains the lowest worst-case disparity on the three datasets, and the methods that coincide at the prediction level separate here.  The supremum over $s$ and over the discriminators constrains the representation at every sensitive value, whereas an average over $S$, or a penalty on the fitted head, leaves a dependence on $S$ that another head can exploit.
...
\paragraph{Two-layer heads.}  The heads of $\mathcal{F}_c$ are linear in the representation.  Writing
\begin{equation*}
...
This appendix examines three choices in the penalty of supIPM-FRL on the tabular datasets: the shape of the smoothing kernel, the class of the discriminators and the number $T$ of candidate sensitive values.  Section~\ref{sec:exp-real} uses the Gaussian kernel, the ReLU class $\mathcal{V}_{\mathrm{R}}$ and $T=17$.  Each ablation varies one choice, holds every other setting of Appendix~\ref{app:real-settings} fixed, and leaves the measures of Appendix~\ref{app:worst-case} unchanged.  In each figure the columns are the datasets, the top row reports the prediction-level disparity and the bottom row the worst-case downstream disparity over $\mathcal{F}_1$.
