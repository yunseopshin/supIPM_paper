...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.88\textwidth]{main/fig2g_own_head_all.pdf}
  \vspace{-10pt}
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.88\textwidth]{main/fig3g_worst_head_all.pdf}
  \vspace{-10pt}
...
\paragraph{Computational cost.}  The ReLU discriminators make the penalty a sum over the observations, so supIPM-FRL is trained on mini-batches with unbiased gradients, as shown in Appendix~\ref{app:mini-batch}.  The MMD of FREM is a double sum over pairs, and on the whole training sample it exceeds the memory of the GPU.  Table~\ref{tab:cost} gives one gradient step on Adult, and Appendix~\ref{app:real-cost} the cost as the mini-batch grows.



\subsection{Ablation studies and additional results}

Appendix~\ref{app:real-additional} compares the methods under five further fairness measures and on the graph datasets.  Appendix~\ref{app:ablation-studies} ablates three choices in the penalty, (i) the shape of the smoothing kernel, (ii) the class of the discriminators and (iii) the number of candidate sensitive values.
